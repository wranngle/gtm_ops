 ...
 * @param {import('./schemas/business_profile.schema.js').BusinessProfile} profile
 * @returns {Promise<import('./schemas/business_profile.schema.js').BusinessProfile>}
 */
...
async function tryProviders(domain) {
  ...
  if (n8nUrl) {
    ...
    if (!n8nSecret) {
      console.warn('N8N_ENRICHMENT_WEBHOOK_URL is set but N8N_WEBHOOK_SECRET is missing; skipping n8n enrichment.');
    } else {
