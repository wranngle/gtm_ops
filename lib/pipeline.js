...
// Core utilities
import { generateProjectIdentity, formatCurrency, formatDateDisplay } from './project_identity.js';
import { ensureDir, generateOutputPath, generateRelatedPaths, slugify } from './file_utils.js';
import { generateHeaderHTML, generateFooterHTML, initSharedComponents } from './shared_components.js';

// Schema v2 compatibility
import { expandLegacyPaths } from './schema_compat.js';
import { buildTemplateContext } from './template_context.js';

// Business profile enrichment
import { getCompanySizeSegment } from './schemas/business_profile.schema.js';
...
// Research
import { researchAllIntegrations, getLibraryStatus, generateResearchGapReport, getResearchDerivedBaseHours } from './integration_research.js';
import { generateResearchPlan, quickTierAssessment } from './research.js';
import { buildTechnicalApproach } from './build_technical_approach.js';
import { performProactiveResearch, mergeResearchResults, generateResearchSummary } from './proactive_research.js';

// System Intelligence (unified lookup)
import { getAllSystemIntelligence, getCatalogBaseline } from './system_intelligence.js';

// Product Detection (AI Voice Agent vs. Project)
import { detectProductType } from './product_detector.js';

// Estimation & Pricing
import { generateEstimate, quickEstimate, extractVolumeFromIntake, initEstimate } from './estimate.js';
import { calculatePricing, calculateProductPricing, calculateROI, initPricing } from './pricing_calculator.js';
import { buildPhases, buildProductPhases, initMilestoneBuilder } from './milestone_builder.js';

// Transforms (consolidated TypeScript module - tsx handles .ts imports)
import {
  transformToProjectPlan,
  renderToHtml as renderProjectPlanHtml,
  buildProposal,
  getPlaceholderPaths,
...
import { LLMExecutor, BatchLLMExecutor } from '../src/services/llm.ts';
import { runFinalHtmlPass as polishHTML } from './html_polish.js';

// Validation & Output
import { validateReport, formatErrors, initValidation } from './validate.js';
import { generatePDF } from './pdf_generator.js';
import { validateBleedOutputGate, validateTierGate, validateExtractionGate } from './schema_validation.js';

// Display Field Sync (prevents display field desync bug)
import { regenerateFinopsDisplayFields } from './display_fields.js';
...
import {
  ...
  mapFormToIntake,
  validateFormData
} from './questionnaire_transform.js';
import {
...
function formatProjectPlanForRender(plan) {
  ...
  if (formatted.finops) {
    ...
    if (formatted.finops.roi) {
      if (typeof formatted.finops.roi.monthly_value === 'number') {
...
export class UnifiedPipeline {
  ...
  async runStructuredExtract(inputPath) {
    ...
    let leadQualification, keyMetrics, companyProfile;
    ...
    this.rawInputOpening = rawContent.substring(0, 1000) + (rawContent.length > 1000 ? '...' : '');
    ...
    this.provenance.field_sources['lead_qualification'] = { source: 'calculated', formula: 'lead_scoring.js' };
   ...
   * @private
   */
  _buildMeasurementsFromForm(formData, intake) {
  ...
  async runExtract(inputPath) {
    ...
    this.rawInputOpening = rawText.substring(0, 1000) + (rawText.length > 1000 ? '...' : '');
  ...
  async runResearch() {
    ...
    try {
      ...
      this.log.warn(`System intelligence lookup failed: ${err.message}`);
  ...
  async runProjectPlan() {
    ...
    const effectiveBaseHours = researchHours > tierHours ? researchHours : tierHours;
    ...
    if (estimate?.finops && extractedBleed > 0) {
      ...
      if (!estimate.finops.value_breakdown.modeled_opportunity ||
        ...
        const maxMonthlyOpp = 50000; // Hard cap at $50K/month
        let wasCapped = false;
        let capReason = '';
  ...
  async runProposal() {
    ...
    if (isProduct) {
      ...
      if (this.businessProfile?.employee_count) {
        const { getCompanySizeSegment } = await import('./schemas/business_profile.schema.js');
    ...
    let milestones;
    if (isProduct && productPricing) {
      milestones = await buildProductPhases(this.schema.intake, productPricing);
    } else {
      milestones = await buildPhases(proposalData, pricing);
    }
    ...
    if (this.businessProfile?.revenue_estimate) {
      const { parseRevenueEstimate } = await import('./schemas/business_profile.schema.js');
  ...
  async runAuditReport() {
    ...
    if (auditReport?.scorecard) {
      const scorecard = auditReport.scorecard;
  ...
  async runLLMFill() {
    ...
    const estimatedCost = (totalTokens / 1000000) * 0.075; // Rough Gemini Flash pricing
  ...
  async runRender(outputDir) {
    ...
    const templateData = {
      ...
      _has_savings: !!(this.schema.pricing?.audit_credit || this.schema.pricing?.early_adopter_discount),
      ...
      raw_input: {
        opening: this.rawInputOpening || '',
        stats: this.rawInputStats || { lines: 0, words: 0, characters: 0 },
        has_content: !!(this.rawInputOpening && this.rawInputOpening.length > 0)
      },
    ...
    if (fs.existsSync(salesStrategyPath)) {
      try {
        ...
        const { validateSalesStrategy } = await import('./schemas/sales_strategy.schema.js');
        ...
        this.log.data('Sales Strategy', `Failed to load: ${err.message}`);
      }
    } else {
    ...
    if (fs.existsSync(internalTemplatePath)) {
      ...
      if (salesStrategy) {
        ...
        if (salesStrategy.pricing_strategy && salesStrategy.pricing_strategy.packages) {
            const packages = salesStrategy.pricing_strategy.packages;
  ...
  async run(inputPath, outputDir) {
    ...
    const STAGE_RETRY_CONFIG = {
      ...
      polish:     { maxAttempts: 4, baseDelayMs: 10000 },  // LLM polish passes
    ...
    try {
      ...
      if (this.businessProfile?.company_url) {
        try {
          ...
          this.log.warn(`Business profile enrichment failed: ${err.message}`);
        }
      }
      ...
      if (intake?.prepared_for?.contact_email || intake?.prepared_for?.contact_name) {
        try {
          ...
          this.log.warn(`Person enrichment failed: ${err.message}`);
        }
      }
      ...
      if (this.businessProfile?.industry) {
        try {
          ...
          const industryTag = this.businessProfile.industry.toLowerCase().replace(/[\s_]+/g, '-');
