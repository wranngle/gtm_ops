 ...
 */
export function generateMilestones(cost, options = {}) {
  const context = loadAgencyContext();
...
export function recommendRetainer(intake, cost) {
  ...
  let tierKey;
  if (projectTier === 'enterprise' || cost.total > 25000) {
...
export async function generateEstimate(intake, research = null) {
  ...
  const riskMultiplier = effort.risk_multiplier || research?.riskMultiplier || 1.0;
...
export function determinePaymentStructure(totalCost) {
  ...
  const upfrontMax = thresholds.upfront_max || 10000;
...
export function extractVolumeFromIntake(intake) {
  ...
  if (intake?.section_b_volume_timing?.q06_runs_per_period) {
    ...
    const parsedVolume = parseFloat(String(rawVolume).replace(/[^0-9.-]/g, ''));
...
export function calculateModeledOpportunity(config = {}, intake = null) {
  ...
  const maxMonthly = config.max_modeled_opportunity_monthly || 50000;
  let wasCapped = false;
  let capReason = '';
...
export function calculateRiskAnalysis(effort, finops, cost) {
  ...
  const riskMultiplier = effort.risk_multiplier || 1.0;
  ...
  if (annualValue > 500000) rewardScore += 2;
  else if (annualValue > 100000) rewardScore += 1;
