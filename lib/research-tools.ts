 ...
 * - Tavily (AI Search)
 *
 * Used by proactive-research when API keys are available.
 *
 * Ported from unified-presales-report lib/research_tools.js (snake_case → kebab-case,
 * JS → TS) to preserve the Exa/Tavily integration during the gtm_ops port. Wired into
 * researchIntegrationWithLLM (lib/proactive-research.ts): when EXA_API_KEY /
 * TAVILY_API_KEY are set, retrieved results are injected into the research prompt as
 * primary evidence; with no keys configured it degrades to a silent no-op (empty
 * findings, no network).
 */
...
export type DeepResearchFindings = {
  sources: Array<ExaSearch | TavilySearch>;
  context: string;
};
 ...
 */
async function nativeRequest(url: string, options: NativeRequestOptions = {}): Promise<any> {
 ...
 */
export async function searchExa(query: string, apiKey: string): Promise<ExaSearch | null> {
  ...
  try {
    ...
    console.warn('[ResearchTools] Exa search failed:', message);
    return null;
  }
}
 ...
 * Search Tavily
 */
export async function searchTavily(query: string, apiKey: string): Promise<TavilySearch | null> {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.warn('[ResearchTools] Tavily search failed:', message);
    return null;
  }
}
...
): Promise<DeepResearchFindings> {
  const findings: DeepResearchFindings = {
    sources: [],
    context: ''
  };
