...
async function measureAndScaleSheets(page, options = {}) {
  ...
  return await page.evaluate((config) => {
    ...
    sheets.forEach((sheet, index) => {
      ...
      const hasPageCard = !!contentContainer;
      ...
      let scale = naturalHeight > availableHeight
      ...
      const finalScale = Math.max(roundedScale, 0.60);
    ...
    styleEl.textContent = `
      /* PDF Dynamic Scaling - Generated by pdf_generator.js */
      @media print {
        ${scaleRules.join('\n')}
...
async function applyPrintFixes(page, options = {}) {
  ...
  await page.evaluate((hideInternal) => {
    ...
    if (hideInternal) {
      document.querySelectorAll('.sheet.internal, #report-internal-strategy').forEach(el => {
