    ...
    return new Promise((resolve, reject) => {
      this.db.serialize(() => {
        ...
        this.db.run(sql, params, function(err) {
          ...
          // ID is fetched by slug below (safer than this.lastID across update vs. insert).
        });
...
export async function syncDatabase() {
  ...
  if (existsSync(indexJsonPath)) {
    try {
      ...
      console.error('[ResearchDB] Error reading library-index.json:', error.message);
    }
  }
  ...
  for (const file of jsonFiles) {
    try {
      ...
      console.warn(`[ResearchDB] Skipping invalid JSON file ${file}:`, error.message);
    }
  }
