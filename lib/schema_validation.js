 ...
 *
 * Integrates Zod schemas with the existing JS pipeline.
...
// These mirror src/schemas/*.ts but in plain JS
// =============================================================================
 ...
 * 
 * @param {Object} inputs - Bleed inputs from estimate calculation
 ...
 * 
 * @param {Object} bleed - Bleed calculation result
 * @param {Object} options - Validation options
 ...
 * @param {Object} measurements - Extracted measurements data
 * @param {Object} options - Validation options
...
export function validateExtractionGate(intake, measurements, options = {}) {
  ...
  if (!intakeResult.success) {
    intakeResult.error.issues.forEach(issue => {
  ...
  if (!measurementsResult.success) {
    measurementsResult.error.issues.forEach(issue => {
 ...
 *
 * @param {Object} tier - Tier assessment from research
 * @param {Object} options - Validation options
 * @returns {Object} Validation result
 */
 ...
 * @param {string} label - Description for errors
 ...
 * 
 * @param {Function} fn - The calculation function
 * @param {Object} inputSchema - Zod schema for inputs
 * @param {Object} outputSchema - Zod schema for outputs (optional)
...
export function withValidation(fn, inputSchema, outputSchema = null) {
  return function validatedFn(...args) {
    ...
    if (inputSchema && args[0]) {
      const inputResult = inputSchema.safeParse(args[0]);
