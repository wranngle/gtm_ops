...
export class AdminManager {
  ...
  async recordMetric(workspaceId, metricType, value = 1) {
    ...
    const bucketDate = Math.floor(now / (24 * 60 * 60 * 1000)) * (24 * 60 * 60 * 1000);
   ...
   * @returns {Promise<Array>}
   */
  async getTopUsers(workspaceId, metricType = MetricType.DOCUMENTS_CREATED, period = TimePeriod.THIS_MONTH, limit = 10) {
    const { start, end } = getTimestampRange(period);
