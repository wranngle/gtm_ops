...
export class UsageTracker {
      ...
      `, params, (err, counts) => {
        ...
        `, params, (err, tokens) => {
          ...
          `, params, (err, timing) => {
            ...
            const estimatedCost =
              (inputTokens / 1000000) * COST_CONFIG.gemini.inputCostPer1M +
              (outputTokens / 1000000) * COST_CONFIG.gemini.outputCostPer1M +
  ...
  async getCostBreakdown(options = {}) {
    ...
    const geminiInputCost = (summary.total_input_tokens / 1000000) * COST_CONFIG.gemini.inputCostPer1M;
    const geminiOutputCost = (summary.total_output_tokens / 1000000) * COST_CONFIG.gemini.outputCostPer1M;
    ...
    return {
      gemini: {
        ...
        input_cost: Math.round(geminiInputCost * 10000) / 10000,
        output_cost: Math.round(geminiOutputCost * 10000) / 10000,
        total_cost: Math.round((geminiInputCost + geminiOutputCost) * 10000) / 10000
      },
      puppeteer: {
        pdf_pages: summary.total_pdf_pages,
        total_cost: Math.round(puppeteerCost * 10000) / 10000
      },
      total_estimated_cost: summary.estimated_cost,
      cost_per_execution: summary.total_executions > 0
        ? Math.round((summary.estimated_cost / summary.total_executions) * 10000) / 10000
        : 0
    };
  }
