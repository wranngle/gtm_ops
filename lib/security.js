...
const MAX_INPUT_LENGTH = 500000; // 500k characters
...
export function maskApiKeysInText(text) {
  ...
  return text
    .replace(/AIza[A-Za-z0-9_-]{20,}/g, (match) => `AIza...${match.slice(-4)}`)
    .replace(/gsk_[A-Za-z0-9]{20,}/g, (match) => `gsk_...${match.slice(-4)}`)
    .replace(/sk-[A-Za-z0-9]{20,}/g, (match) => `sk-...${match.slice(-4)}`);
}
