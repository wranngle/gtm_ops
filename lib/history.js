...
export class HistoryManager {
      ...
      columns.forEach(col => {
        this.db.run(`ALTER TABLE executions ADD COLUMN ${col.name} ${col.type}`, (err) => {
          if (err && !err.message.includes('duplicate column name')) {
            // Silence duplicate column errors, but log others
            if (!err.message.includes('duplicate')) console.error(`Migration error for ${col.name}:`, err.message);
          }
        });
      });
  ...
  async startExecution(projectIdentity, inputPath, inputHash) {
      ...
      const revision = match ? parseInt(match[1], 10) : 1;
      const now = Date.now();
