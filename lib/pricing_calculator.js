...
export function assessComplexity(auditData, options) {
  ...
  if (companySizeSegment && COMPLEXITY_MULTIPLIERS.company_size?.segments) {
    complexity.company_size = COMPLEXITY_MULTIPLIERS.company_size.segments[companySizeSegment]?.multiplier || 1.0;
  }
...
const DEFAULT_PRICING_VALIDATION = {
  ...
  max_modeled_opportunity_monthly: 50000  // Cap at $50K/month to prevent unrealistic projections
};
 ...
 */
function calculateSetupFee(intake, options = {}) {
  const breakdown = [];
