...
function getTimestampRange(period) {
  const now = new Date();
  // +1ms makes SQL `created_at < end` behave like `created_at <= now` at the
  // millisecond boundary. Without this, an event recorded with timestamp T
  // followed immediately by a query with end=T was excluded — race observed
  // in tests/unit/admin.test.ts "should get top users by activity" where
  // back-to-back logActivity()+getTopUsers() landed on the same Date.now() ms.
...
export class AdminManager {
  ...
  async getActivityFeed(workspaceId, { limit = 20, offset = 0, userId = null } = {}) {
    ...
    // rowid tiebreaker: when two logActivity() calls land on the same
    // Date.now() ms, created_at ties and SQLite's row order is undefined.
    // SQLite's implicit rowid auto-increments per insert — stable order.
  ...
  async getSystemHealth() {
    const rows = await this._all(`
      ...
      WHERE h1.rowid = (
        ...
        ORDER BY h2.captured_at DESC, h2.rowid DESC
        LIMIT 1
      )
    `);
  ...
  async getTopUsers(workspaceId, _metricType = MetricType.DOCUMENTS_CREATED, period = TimePeriod.THIS_MONTH, limit = 10) {
    ...
    const rows = await this._all(
      `SELECT user_id, COUNT(*) as activity_count
       ...
       GROUP BY user_id
       ORDER BY activity_count DESC, MAX(rowid) DESC
       LIMIT ?`,
