...
export function parseResearchMarkdown(markdown) {
  ...
  if (confMatch) research.confidence = parseInt(confMatch[1]) / 100;
  ...
  if (detailsSectionMatch) {
    ...
    for (const section of integrationSections) {
      ...
      const normalizedKey = integrationName.toLowerCase().replace(/\s+/g, '-');
      ...
      if (complexityMatch) {
        ...
        const rawScore = parseInt(complexityMatch[1]);
  ...
  if (nodesMatch) {
    const min = parseInt(nodesMatch[1]);
    const max = nodesMatch[2] ? parseInt(nodesMatch[2]) : min;
    research.complexity.estimated_nodes = Math.round((min + max) / 2);
  }

  // Extract labor factors table
  const laborMatch = markdown.match(/## Labor Factors[\s\S]+?\|[\s\S]+?\|[\s\S]+?\|([\s\S]+?)(?=\n---|\n##)/);
  if (laborMatch) {
    const rows = laborMatch[1].split('\n').filter(r => r.includes('|'));
    for (const row of rows) {
      const cols = row.split('|').map(c => c.trim()).filter(c => c);
      // Skip header row and separator rows (e.g., |----|----|)
      if (cols.length >= 3 && cols[0] !== 'Factor' && !cols[0].match(/^-+$/)) {
  ...
  // Extract risks table
  const risksMatch = markdown.match(/## Risks & Mitigations[\s\S]+?\|[\s\S]+?\|[\s\S]+?\|([\s\S]+?)(?=\n---|\n##)/);
  if (risksMatch) {
    const rows = risksMatch[1].split('\n').filter(r => r.includes('|'));
    for (const row of rows) {
      const cols = row.split('|').map(c => c.trim()).filter(c => c);
      // Skip header row and separator rows (e.g., |----|----|)
      if (cols.length >= 4 && cols[0] !== 'Risk' && !cols[0].match(/^-+$/)) {
        research.risks.push({
          risk: cols[0],
          likelihood: cols[1],
          impact: cols[2],
  ...
  if (baseHoursMatch) {
    research.effort_recommendation.base_hours = parseInt(baseHoursMatch[1]);
  }
...
export function calculateFreshness(researchDate) {
  ...
  return { stale: false, days: daysSince, score: 1.0 - (daysSince / 90) };
}
...
export async function getCachedResearch(integrationName) {
  ...
  try {
    ...
    console.warn(`[ResearchDB] Lookup failed for ${integrationName}:`, e.message);
