...
/**
 * LLM prompt for researching a single integration
 * Uses n8n node database for context, LLM fills in details
 */
const INTEGRATION_RESEARCH_PROMPT = `You are a technical researcher analyzing API integrations for n8n workflow development.

Research the following integration and provide structured assessment:

Integration Name: {{integration_name}}
Context: This integration will be used in an automation workflow via n8n.

{{#if native_node_context}}
KNOWN n8n NODE INFO (use this as baseline):
{{native_node_context}}
{{/if}}
...
Provide your analysis as JSON with these EXACT fields:
{
  ...
  "has_native_n8n_node": {{has_native_node}},
  "native_node_name": {{native_node_name}},
  "auth_type": "{{auth_type}}",
  "auth_complexity": "simple" | "moderate" | "complex",
  "auth_notes": "Brief description of authentication requirements",
  "api_quality": "{{api_quality}}",
  "api_documentation_url": "https://..." or null,
  "rate_limits": "Description of rate limits or 'unknown'",
  ...
  "operations_available": ["operation1", "operation2"],
  "similar_integrations": ["Related system 1", "Related system 2"],
  "confidence": {{confidence}},
  "research_notes": "{{research_notes}}"
}

IMPORTANT GOTCHAS TO CHECK FOR:
- OAuth2 scope mismatches (common with HubSpot, Salesforce)
- Rate limit variations by tier/plan
- Deprecated API versions
- Webhook payload format differences
- Timezone handling issues
- Required field changes in recent API versions
...
/**
 * Generate markdown research report following n8n-methodology format
 */
function generateMarkdownReport(research, clientName = 'Unknown', workflowName = 'Unknown') {
  const date = new Date().toISOString().split('T')[0];
  const confidence = Math.round((research.confidence || 0.7) * 100);

  return `# ${research.integration_name || research.integration} Integration Research

**Business Process**: ${workflowName}
**Research Date**: ${date}
**Researcher Confidence**: Medium (${confidence}%)
...
This integration analysis was auto-generated during pipeline execution because no cached research was available. For production use, invoke the technical-research skill manually for more thorough research using MCP tools.

${research.research_notes || 'No additional notes.'}
...
|-------------|-------------|-----------|----------------|------------|
| ${research.integration_name || research.integration} | ${research.has_native_n8n_node ? 'Yes' : 'No'} | ${research.auth_type || 'unknown'} | ${research.api_documentation_url ? 'Yes' : 'No'} | ${confidence}% |

---

## Integration Details

### 1. ${research.integration_name || research.integration}
...
**Key Gotchas**:
${(research.gotchas || []).map(g => `- ${g}`).join('\n') || '- No known issues'}

**Rate Limits**: ${research.rate_limits || 'Unknown'}

**Client Must Provide**:
${(research.client_must_provide || []).map(c => `- ${c}`).join('\n') || '- API credentials'}

**Complexity Score**: ${research.complexity_score || 5}/10 → ${research.complexity_tier || 'moderate'}

**Operations Available**:
${(research.operations_available || []).map(o => `- ${o}`).join('\n') || '- Standard CRUD operations'}

---

## Complexity Analysis

### Overall Score: ${research.complexity_score || 5}/10 → ${research.complexity_tier || 'moderate'}

**Estimated Nodes**: ${research.estimated_hours ? Math.ceil(research.estimated_hours / 2) : 10}

**Contributing Factors**:
- ${research.has_native_n8n_node ? 'Has native n8n node (-2)' : 'No native node, HTTP required (+2)'}
- ${research.auth_type === 'oauth2' ? 'OAuth2 authentication (+1)' : 'Standard authentication (+0)'}
- ${research.webhook_support ? 'Webhook support available (-1)' : 'No webhook support (+1)'}
...
| Factor | Impact | Notes |
|--------|--------|-------|
| Native node availability | ${research.has_native_n8n_node ? 'Low' : 'High'} | ${research.has_native_n8n_node ? 'Reduces development time' : 'Requires HTTP Request configuration'} |
| Auth complexity | ${research.auth_complexity === 'complex' ? 'High' : research.auth_complexity === 'moderate' ? 'Medium' : 'Low'} | ${research.auth_type} authentication |
| API quality | ${research.api_quality === 'poor' ? 'High' : research.api_quality === 'fair' ? 'Medium' : 'Low'} | ${research.api_quality || 'Unknown'} documentation |
...
|------|------------|--------|------------|
| API changes | Medium | Medium | Pin to stable API version |
| Rate limiting | ${research.rate_limits && research.rate_limits !== 'unknown' ? 'Low' : 'Medium'} | Medium | Implement retry logic with backoff |
| Auth expiration | Medium | High | Implement token refresh handling |

---

## Effort Recommendation

**Tier**: ${research.complexity_tier || 'moderate'}
**Base Hours**: ${research.estimated_hours || 8}

**Rationale**:
${research.has_native_n8n_node
    ? `n8n has native node support for ${research.native_node_name || 'this integration'}, reducing implementation complexity.`
    : 'No native n8n node available. Will require HTTP Request node with custom authentication handling.'}

**Caveats**:
- This research was auto-generated; verify findings before final estimation
- Actual complexity may vary based on specific use case requirements
${research.gotchas?.slice(0, 2).map(g => `- ${g}`).join('\n') || ''}

---

## Citations

${research.api_documentation_url ? `1. ${research.api_documentation_url} - Official API documentation` : '- No citations available (auto-generated research)'}
`;
}
...
export async function researchIntegrationWithLLM(integrationName, forceLLM = false) {
  ...
  let prompt = `You are a technical researcher analyzing API integrations for n8n workflow development.
...
`;
  ...
  try {
    ...
    console.warn('[ResearchDB] Warning: Could not initialize DB connection', e.message);
  }
  ...
  for (const result of results) {
    ...
    if (db) {
      try {
        ...
        console.warn(`   ⚠ Failed to index ${name} in SQLite:`, err.message);
      }
    }
  }
...
export function mergeResearchResults(existingResearch, proactiveResearch) {
  ...
  for (const proactive of proactiveResearch) {
    ...
    if (existingIndex >= 0) {
      if (!merged[existingIndex].research?.found) {
