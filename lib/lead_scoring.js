...
export function getKeyMetrics(formData, qualification) {
  ...
  const integrationComponent = qualification.components.find(c => c.name === 'integration_complexity');
  const rawScore = integrationComponent?.raw_score ?? 50;
  const complexityScore = Math.round((100 - rawScore) / 10);
