...
type LibraryIndex = {
  research_files: Record<string, LibraryIndexMeta>;
};

type JsonResearchContent = {
  title?: string;
  integration?: string;
  integrations?: string[];
...
export class ResearchDB {
  ...
  async getResearchByIntegration(integrationName: string): Promise<ResearchEntryRow | undefined> {
    return new Promise((resolve, reject) => {
      const query = `
        SELECT r.*
        ...
        LIMIT 1
      `;
      this.db.get<ResearchEntryRow>(query, [integrationName.toLowerCase()], (err, row) => {
        if (err) reject(err);
        else resolve(row);
      });
    });
  ...
  async addEntry(entry: ResearchEntry, integrations: string[] = []): Promise<number> {
    return new Promise((resolve, reject) => {
      this.db.serialize(() => {
...
/**
 * Sync database from library-index.json and independent JSON files
 */
export async function syncDatabase(): Promise<void> {
  ...
  if (existsSync(indexJsonPath)) {
    try {
      const index: LibraryIndex = JSON.parse(readFileSync(indexJsonPath, 'utf-8'));
  ...
  for (const file of jsonFiles) {
    try {
      const content: JsonResearchContent = JSON.parse(readFileSync(join(N8N_RESEARCH_PATH, file), 'utf-8'));
