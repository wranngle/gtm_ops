...
export function buildProductPhase2(intake, productPricing, options = {}) {
  ...
  const allocations = {
    configuration: { amount: Math.round(setupFee * 0.20), percentage: 20 },
    integration: { amount: Math.round(setupFee * 0.45), percentage: 45 },
    training: { amount: Math.round(setupFee * 0.15), percentage: 15 },
    launch: { amount: setupFee - Math.round(setupFee * 0.20) - Math.round(setupFee * 0.45) - Math.round(setupFee * 0.15), percentage: 20 }
  };
