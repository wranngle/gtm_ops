...
async function seedFromFixtures(database) {
  ...
  const stmt = database.prepare(`INSERT OR IGNORE INTO case_studies (id, vendor, problem_json, solution_json, meta_json) VALUES (?, ?, ?, ?, ?)`);
  let count = 0;
  for (const file of files) {
    try {
      const data = JSON.parse(fs.readFileSync(path.join(FIXTURES_DIR, file), 'utf-8'));
      const id = data.id || generateCaseStudyId(data);
      stmt.run(id, data.vendor || 'unknown', JSON.stringify(data.problem || {}), JSON.stringify(data.solution || {}), JSON.stringify(data.meta || {}));
      count++;
