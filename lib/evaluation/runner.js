...
async function executePipeline(maskedIntake, options = {}) {
  const { timeout = 120000, outputDir = null } = options;
        ...
        if (jsonMatch) {
          ...
          if (outputFiles.length > 0) {
            ...
            const output = JSON.parse(
              fs.readFileSync(path.join(tempDir, latestFile), 'utf-8')
            );
            resolve({
              success: true,
              output,
...
export async function runBatchEvaluation(options = {}) {
  const {
    concurrency = 1, // Sequential by default (LLM rate limits)
    scoringConfig = DEFAULT_SCORING_CONFIG,
  ...
  if (scores.length > 0) {
    summary.mean_score = parseFloat((scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(2));
    ...
    summary.top_flaws = Object.entries(flawCounts)
      ...
      .map(([code, count]) => ({
        code,
        count,
        percent: parseFloat(((count / completed.length) * 100).toFixed(1)),
      }));
  }
...
export async function checkReadiness() {
  ...
  // Check pipeline
  try {
    const pipelinePath = path.join(PROJECT_ROOT, 'lib', 'pipeline.js');
