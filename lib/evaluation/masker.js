...
export function validateNoSolutionLeakage(intake, solution) {
  ...
  for (const integration of solution.integrations || []) {
    ...
    if (systemName && intakeText.includes(systemName.toLowerCase())) {
      // This is OK - systems can be in both problem and solution
      // Only flag if it's integration-specific details from legacy format
      if (typeof integration === 'object' && integration.integration_type && intakeText.includes(integration.integration_type)) {
...
export function generateMaskingReport(caseStudy, maskedIntake) {
  ...
  return {
    ...
    solution_hidden: {
      ...
      has_pricing: !!(solution.pricing_model || solution.price_min || solution.price_max),
      has_roi: !!(solution.roi_achieved || solution.roi_months),
    },
  };
}
