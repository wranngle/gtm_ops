...
import {
  ...
  generateCaseStudyId,
} from '../schemas/case-study.schema.js';
...
async function seedFromFixtures(database) {
  ...
  try {
    ...
    console.warn('Eval corpus: could not prepare seed insert:', error.message);
    return;
  }
  ...
  for (const file of files) {
    try {
      ...
      console.warn(`Eval corpus: skipped malformed fixture ${file}: ${error.message}`);
    }
  }
...
export async function createCaseStudy(data) {
  ...
  // Validate input
  const parseResult = CreateCaseStudySchema(data);
  if (parseResult instanceof type.errors) {
    throw new Error(`Invalid case study data: ${parseResult.summary}`);
  }

  const caseStudy = parseResult;
...
export async function updateCaseStudyMeta(id, metaUpdate) {
  ...
  // Validate update shape (defaults from the schema must NOT clobber existing
  // fields the caller didn't pass — merge from the original `metaUpdate` below).
  const parseResult = UpdateCaseStudyMetaSchema(metaUpdate);
  if (parseResult instanceof type.errors) {
    throw new Error(`Invalid meta update: ${parseResult.summary}`);
  }
  ...
  // Merge updates
  const updatedMeta = { ...existing.meta, ...metaUpdate };
    ...
    database.get(
      ...
      (err, row) => {
        ...
        resolve({
          ...
          avgQuality: row.avg_quality ? Number.parseFloat(row.avg_quality.toFixed(2)) : null,
          oldest: row.oldest,
          newest: row.newest,
        });
      }
