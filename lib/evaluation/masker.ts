...
import { IntakeSchema } from '../schemas/intake.schema.js';
import { CaseStudyProblemSchema } from '../schemas/case-study.schema.js';
...
export function toIntake(caseStudy, options = {}) {
  ...
  if (isFactoryFormat) {
    ...
    const problemValidation = CaseStudyProblemSchema(problem);
    if (problemValidation instanceof type.errors) {
      throw new Error(`Invalid problem structure: ${problemValidation.summary}`);
    }
  ...
  if (validateOutput) {
    const validation = IntakeSchema(intake);
    if (validation instanceof type.errors) {
      console.warn('[MASKER WARNING] Generated intake has validation issues:');
      for (const issue of validation as any) {
        const path = Array.isArray(issue.path) ? issue.path.join('.') : String(issue.path ?? '');
        console.warn(`  - ${path}: ${issue.message ?? String(issue)}`);
      }
    }
  }
