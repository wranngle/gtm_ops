...
import { DEFAULT_SCORING_CONFIG } from '../schemas/evaluation.schema.ts';
...
function systemNamesMatch(name1, name2) {
  ...
  const words1 = norm1.replace(/[0-9]/g, '').trim();
  const words2 = norm2.replace(/[0-9]/g, '').trim();
...
function jaccardSimilarity(setA, setB) {
  ...
  if (a.length === 0 && b.length === 0) return 1.0; // Both empty = match
  if (a.length === 0 || b.length === 0) return 0.0; // One empty = no match
...
const AGENT_COMPATIBILITY = {
  inbound: { inbound: 1.0, hybrid: 0.5, outbound: 0.0 },
  outbound: { outbound: 1.0, hybrid: 0.5, inbound: 0.0 },
  hybrid: { hybrid: 1.0, inbound: 0.5, outbound: 0.5 },
};
...
export function scorePricingReasonableness(pipelinePrice, groundTruthPrice, config = {}) {
  ...
  // Convert to numbers
  const pipeline = parseFloat(pipelinePrice);
  const truth = parseFloat(groundTruthPrice);
...
export function scoreTimelineRealism(pipelineWeeks, groundTruthWeeks, config = {}) {
  ...
  const pipeline = parseFloat(pipelineWeeks);
  const truth = parseFloat(groundTruthWeeks);
...
export function scoreFeatureCoverage(pipelineFeatures, groundTruthFeatures) {
  ...
  if (truth.length === 0) {
    return {
      score: 1.0, // No features to match = full score
...
export function compare(pipelineOutput, groundTruthSolution, config = DEFAULT_SCORING_CONFIG) {
  ...
  const extractFromPipeline = () => {
    ...
    if (!integrations.length || integrations.every((i) => !i)) {
    ...
    // Note: project_type like "voice_agent" needs mapping to inbound/outbound/hybrid
    const projectType = pipelineOutput.intake?.classification?.project_type;
...
export function detectFlaws(scores) {
  ...
  for (const dim of scores.dimensions) {
    ...
    // Tier flaws
    if (dimension === 'tier_match' && details?.diff) {
      if (details.pipeline && details.truth) {
