  ...
  try {
    ...
    if (result.scores?.dimensions) {
      ...
      for (const dimResult of result.scores.dimensions) {
        const label = (dimResult.dimension || 'unknown').replaceAll('_', ' ');
        const scoreVal = dimResult.score == null ? 'N/A' : (dimResult.score * 100).toFixed(0);
        console.log(`    ${label.padEnd(25)}: ${formatScore(Number.parseFloat(scoreVal))} ${c.dim}(${dimResult.rationale || ''})${c.reset}`);
      }
    }
...
async function cmdReport(options = {}) {
  ...
  try {
    ...
    for (const { dim, mean, min, max } of dimResults) {
      const label = dim.replaceAll('_', ' ').padEnd(25);
...
function generateRecommendations(dimResults, sortedFlaws, evalCount) {
  ...
  for (const { dim, mean } of dimResults.slice(0, 2)) {
    if (mean < 60) {
      switch (dim) {
        ...
        default: {
          recommendations.push(`Investigate low ${dim.replaceAll('_', ' ')} scores`);
        }
      }
    }
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push('Pipeline performance within acceptable ranges', 'Continue building corpus to identify edge cases');
  }
