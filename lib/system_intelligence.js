...
import { getCachedResearch, calculateFreshness } from './integration_research.js';
...
function findCatalogMatch(name, systems) {
  ...
  if (idMatch) {
    return { entry: idMatch, matchType: 'exact_id', confidence: 1.0 };
  }

  // Strategy 2: Exact name match (case-insensitive)
  const nameMatch = systems.find((s) => s.name.toLowerCase() === normalizedName);
  if (nameMatch) {
    return { entry: nameMatch, matchType: 'exact_name', confidence: 1.0 };
  }
