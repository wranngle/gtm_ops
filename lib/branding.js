...
export function hexToRgb(hex) {
  ...
  const result = /^#([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})$/i.exec(normalized);
  return result
    ? {
...
export class BrandingManager {
  ...
  async setBranding(workspaceId, updates) {
    ...
    // Check contrast
    if (updates.textColor && updates.backgroundColor) {
      if (!meetsContrastRequirement(updates.textColor, updates.backgroundColor)) {
    ...
    if (existing) {
      ...
      await this._run(
        ...
        [
          ...
          updates.poweredByVisible !== false ? 1 : 0,
  ...
  async applyBranding(workspaceId, html, options = {}) {
    ...
    result = result.replace(/\{\{primary_color\}\}/g, branding.primary_color);
    result = result.replace(/\{\{secondary_color\}\}/g, branding.secondary_color);
    result = result.replace(/\{\{background_color\}\}/g, branding.background_color);
    result = result.replace(/\{\{text_color\}\}/g, branding.text_color);
    result = result.replace(/\{\{success_color\}\}/g, branding.success_color);

    // Replace logo
    if (branding.logo_url) {
      result = result.replace(/\{\{logo_url\}\}/g, branding.logo_url);
    }

    // Handle powered by
    if (!branding.powered_by_visible || branding.white_label) {
      result = result.replace(/\{\{powered_by\}\}/g, '');
    } else {
      result = result.replace(/\{\{powered_by\}\}/g, 'Powered by Wranngle');
    }

    // Custom footer
    if (branding.custom_footer) {
      result = result.replace(/\{\{custom_footer\}\}/g, branding.custom_footer);
    }
