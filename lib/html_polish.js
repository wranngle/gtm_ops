...
// Gemini API base URL (v1beta required for systemInstruction + responseMimeType)
...
async function runFinalHtmlPass(htmlPath, context = {}, options = {}) {
  ...
  try {
    ...
    } else if (correctedHtml.trim() === htmlContent.trim()) {
    ...
    if (!isValidHtml(correctedHtml)) {
      ...
      console.warn('  Response preview:', correctedHtml.substring(0, 200));
...
async function generateWithGemini(systemPrompt, userPrompt, retries = 2) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      const INACTIVITY_TIMEOUT_MS = 45000; // 45 seconds of no activity = abort
...
function manualPolishHTML(html) {
  ...
  while ((match = brokenSentencePattern.exec(originalPolished)) !== null) {
    ...
    if (text.match(/^Operation\s+\w+\s+\w+$/i)) {
      continue;
    }
    ...
    if (words.length <= 6 && words.every(w => /^[A-Z0-9]/.test(w) || /^(a|an|the|and|or|of|to|for|in|on|at|by|with)$/i.test(w))) {
      continue;
    }
