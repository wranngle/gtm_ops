...
/**
 * Resolve a request id from an inbound X-Request-Id header, falling
 * back to a generator (typically `crypto.randomUUID`) when the header
 * is missing or malformed. Pure function so the validation contract
 * is unit-testable independently of Express.
 *
 * Why the regex (`^[\w-]{1,64}$`): we mirror the request id back as a
 * response header, so an attacker-supplied X-Request-Id with newlines,
 * spaces, or extra-long content could either inject more headers
 * (CRLF) or bloat the log line for every request. Clamping to
 * 1–64 bytes of [A-Za-z0-9_-] is wide enough for every common
 * trace-id format (UUID, ULID, Cloudflare ray id, OpenTelemetry
 * 16/32-hex) but narrow enough to neutralize the abuse cases.
 *
 * @param {Object} headers - req.headers (case-insensitive keys)
 * @param {() => string} generateId - fallback id generator
 * @returns {string} validated upstream id, or freshly generated one
 */
export function resolveRequestId(headers, generateId) {
  const upstream = headers?.['x-request-id'];
  if (typeof upstream === 'string' && /^[\w-]{1,64}$/.test(upstream)) {
    return upstream;
  }
