...
/**
 * Build rich integration notes from research details
 * @param {string} systemName - Integration name
 * @param {object} detail - Rich integration detail from parseResearchMarkdown
 * @param {object} fallbackIntegration - Fallback integration data from table
 * @returns {string} Rich notes for the integration
 */
function buildRichNotes(systemName, detail, fallbackIntegration) {
  const noteParts = [];

  // Native node status
  if (detail?.native_node_info) {
    noteParts.push(detail.native_node_info);
  } else if (detail?.native_node) {
    noteParts.push('Native n8n node available');
  } else if (detail?.native_node === false) {
    noteParts.push('Requires HTTP Request node');
  }

  // Authentication
  if (detail?.auth) {
    noteParts.push(`Auth: ${detail.auth}`);
  } else if (fallbackIntegration?.auth_type) {
    noteParts.push(`Auth: ${fallbackIntegration.auth_type}`);
  }

  // Key gotchas (top 2)
  if (detail?.gotchas?.length > 0) {
    const topGotchas = detail.gotchas.slice(0, 2).join('; ');
    noteParts.push(`Key considerations: ${topGotchas}`);
  }

  // Rate limits
  if (detail?.rate_limits) {
    noteParts.push(`Rate limits: ${detail.rate_limits}`);
  }

  // Integration pattern
  if (detail?.integration_pattern) {
    noteParts.push(`Pattern: ${detail.integration_pattern}`);
  }

  // Operations (if native node with many operations)
  if (detail?.operations?.length > 0) {
    const opCount = detail.operations.length;
    const topOps = detail.operations.slice(0, 3).join(', ');
    noteParts.push(`${opCount} operations available (${topOps}...)`);
  }

  // Client requirements (top 3)
  if (detail?.client_must_provide?.length > 0) {
    const reqs = detail.client_must_provide.slice(0, 3).join(', ');
    noteParts.push(`Client must provide: ${reqs}`);
  }

  // NOTE: Complexity score REMOVED from client-facing notes (migrated to Page 8 internal)
  // The complexity data is preserved in integration.complexity_internal for Page 8 rendering
...
function findIntegrationDetail(systemName, integrationDetails) {
  ...
  const normalizedName = systemName.toLowerCase().replace(/\s+/g, '-');
...
function matchGenericCategory(systemName) {
  ...
  for (const key of Object.keys(GENERIC_CATEGORIES)) {
    ...
    if (isMatch) {
      ...
      const firstWordNormalized = firstWord.toLowerCase().replace(/[\/\-]/g, ' ').trim();
      const firstWordParts = firstWordNormalized.split(' ').filter(p => p.length > 0);

      // Generic modifiers that don't indicate a brand
      const genericModifiers = ['my', 'our', 'the', 'a', 'an', 'generic', 'custom', 'internal', 'legacy'];
      ...
      const hasGenericPart = firstWordParts.some(part => 
        genericModifiers.includes(part) || genericDescriptors.includes(part)
      );
 ...
 */
function buildIntegrations(integrationResearch, intake) {
  const integrations = [];
  ...
  for (const item of filteredResearch) {
    ...
    const isGeneric = genericMatch.isGeneric;
  ...
  for (const item of integrationResearch) {
    ...
    for (const citation of research.citations) {
      ...
      try {
        ...
        urlPath = urlObj.pathname.split('/').filter(p => p).slice(0, 2).join('/');
      ...
      let pathDescription = citation.description || urlPath || 'Documentation';
