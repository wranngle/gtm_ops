...
import { redactSecretsDeep } from './security.js';
...
export class AuditLogger {
  ...
  async log(action, resourceType, resourceId, metadata = {}, context = {}) {
    return new Promise((resolve, reject) => {
      ...
      // Redact secret-shaped strings from metadata BEFORE hashing and
      // before INSERT. Audit logs are append-only by design — an
      // accidental sk-ant-* / xoxb-* / ghp_* dropped into metadata by
      // a future route handler would persist forever otherwise. The
      // hash is computed over the redacted payload so verifyIntegrity
      // continues to round-trip through the chain.
      const safeMetadata = redactSecretsDeep(metadata);

      const hash = this.computeHash(logId, action, resourceType, resourceId, safeMetadata, timestamp, this.lastHash);
