...
import { createKeyedCollection, toMustacheCollection } from './collections.js';
...
export class Extractor {
  ...
  recalculateBleedDeterministically(measurements, intake = null) {
    ...
    if (intake?.section_b_volume_timing?.q07_avg_trigger_to_end) {
      const intakeTime = parseFloat(intake.section_b_volume_timing.q07_avg_trigger_to_end);
    ...
    if (intake?.section_b_volume_timing?.q06_runs_per_period) {
      const intakeVolume = parseFloat(intake.section_b_volume_timing.q06_runs_per_period);
