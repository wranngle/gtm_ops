...
import { validateBleedInputsGate } from './schema_validation.js';
...
export class Extractor {
  ...
  async callLLM(systemPrompt, userPrompt) {
    ...
    // LLM responses occasionally come back with malformed JSON
    // (truncation at the token limit, fence variants, runaway strings).
    // Wrap the parse so the caller gets actionable context instead of
    // a bare "Unexpected token" SyntaxError that omits the source.
    ...
    try {
      ...
      const error = new Error(
        ...
        `Tokens used: ${this.stats.tokensUsed}. ` +
