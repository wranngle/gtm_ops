defmodule Symphony.Codex.DynamicTool do
  ...
  The only currently advertised dynamic tool is `linear_graphql`, which
  forwards a raw GraphQL document plus optional variables to the
  configured Linear endpoint using the runtime's tracker auth. The tool
  is only meaningful when `tracker.kind == :linear`; the orchestrator
  layer is responsible for deciding whether to advertise it (see
  `tool_specs/1`).
  ...
  @doc """
  Static list of advertised tool specifications.

  The orchestrator may choose to filter this list by current
  `tracker.kind` so the agent only sees tools that are wired up. The
  spec § 10.5 only defines `linear_graphql` so we always return it; the
  caller decides whether to advertise.
  """
  @spec tool_specs() :: [map()]
  def tool_specs do
    [
