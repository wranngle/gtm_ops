...
| Integration layer | `local_markdown` and `github_issues` tracker adapters |
...
| §10 Agent runner JSON-RPC protocol | NOT implemented — Bash shells out to `agent.command` (default `scripts/bin/llm.sh`) and reads stdout as one final blob | partial — `local_shell` runner; full Codex app-server adapter is TD-007 work |
...
| §13.1 Logging context (issue_id, issue_identifier, session_id, key=value phrasing) | yes — ECS jsonl with `issue.identifier`, `event.action`, `event.outcome`; `session_id` is N/A in one-shot mode | yes — adds `session.id` once Codex app-server lands |
| §13.3 Snapshot interface | NOT applicable (one-shot has no live state) | future work (TD-007) |
| §13.5 Token accounting | NOT applicable | future work (TD-007) |
...
- `WORKFLOW.md` extensions used here that are NOT in the upstream spec: top-level `log_path`, `agent.require_explicit_run`. Both are documented in this file.
