...
| TD-007 | Symphony fidelity | WORKFLOW.md and `symphony.sh` now use the spec's nested YAML schema (`tracker`, `polling`, `workspace`, `hooks`, `agent`, `codex`). Still missing: long-running daemon shape, Codex app-server JSON-RPC protocol, state machine (Unclaimed → Claimed → Running/RetryQueued → Released), reconciliation, retry queue, hooks execution, Liquid templates, token accounting, snapshot interface. | High | Rewrite as a real daemon (likely Python or Elixir per spec reference impl). The shape-correct GitHub Issues worktree at `.claude/worktrees/agent-a0f5c2fc8d56dd894` should be discarded and re-done against the spec. |
