...
test.describe('widget unreachability fallback', () => {
	...
	test('ConvAI wrapper suppresses embedded banner links and leaves only the explicit Agents escape hatch', async ({page}) => {
		...
		await expect.poll(async () => page.locator('a[href*="elevenlabs.io"], a[href*="elevenlabs.com"]').count(), {
			timeout: 5000,
		}).toBe(0);
		await expect(page.locator('[data-testid="agents-elevenlabs-escape"]')).toHaveCount(0);

		const embeddedBanner = await page.locator('elevenlabs-convai').evaluate(element => {
			const root = (element as HTMLElement).shadowRoot;
			const link = root?.querySelector('[data-gtm-suppressed-external-link]') as HTMLElement | undefined;
			const banner = root?.querySelector('[data-gtm-suppressed-external-banner]') as HTMLElement | undefined;
			return {
	...
	test('Agents page shows fallback when unpkg is blocked', async ({page}) => {
		...
		await expect(page.locator('.convai-mount--unreachable')).toBeVisible({timeout: 8000});
	...
	test('coach dock shows fallback when unpkg is blocked', async ({page}) => {
		...
		await expect(page.locator('.coach-dock .convai-mount--unreachable')).toBeVisible({timeout: 8000});
	});
	...
	test('Evals page lab shows fallback when unpkg is blocked', async ({page}) => {
		...
		await expect(page.locator('.eval-convai-frame .convai-mount--unreachable')).toBeVisible({timeout: 8000});
	...
	test('loading state renders before either ready or fallback fires', async ({page}) => {
		...
		const loadingOrReady = await page.evaluate(() =>
			Boolean(document.querySelector('.convai-mount--loading')
	...
	test('Agents page replaces a mounted-but-empty widget shell with local admin recovery', async ({page}) => {
		await page.route('**/unpkg.com/@elevenlabs/convai-widget-embed@latest*', async route => {
			await route.fulfill({
				status: 200,
				contentType: 'text/javascript',
				body: `
          class EmptyConvai extends HTMLElement {
            connectedCallback() {
              const root = this.attachShadow({ mode: 'open' });
              root.innerHTML = '<template></template><style>:host{display:block;width:100%;height:100%;}</style><div class="overlay"></div><p>Powered by <a href="https://elevenlabs.io/agents">ElevenAgents</a></p>';
            }
          }
          customElements.define('elevenlabs-convai', EmptyConvai);
        `,
			});
		});
		await page.addInitScript(() => {
		...
		await expect(page.locator('elevenlabs-convai')).toHaveCount(1, {timeout: 10_000});
		const stalled = page.locator('[data-testid="convai-render-stalled"]');
		await expect(stalled).toBeVisible({timeout: 6000});
		await expect(stalled).toHaveAttribute('role', 'alert');
		await expect(stalled).toContainText(/session shell is empty/i);
		await expect(stalled.locator('a[href*="elevenlabs.io"], a[href*="elevenlabs.com"]')).toHaveCount(0);

		await stalled.getByRole('button', {name: /open local admin/i}).click();
		await expect(page.locator('.tb__crumb--active')).toContainText('Agents');
		await expect(page.locator('.agent-admin-quick__button:has-text("Context")')).toHaveAttribute('data-active', 'true');
		await expect(page.locator('.agent-admin-tab:has-text("Context")')).toHaveAttribute('aria-selected', 'true');
		const ctx = await page.evaluate(() => (globalThis as any).AppContext.get());
		expect(ctx.extra.selected_agent_key).toBe('sales_coach');
		expect(ctx.extra.agent_admin_panel).toBe('context');
		expect(ctx.extra.triggered_from).toBe('convai-render-stalled');
	});
});
