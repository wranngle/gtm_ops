...
test('Generate proposal flow explains the review gate and destination before execution', async ({openConsole}) => {
	...
	await expect(page.locator('.generate-review-card .artifact-review__packet')).toContainText(/bound to evidence/i);
	await expect(page.locator('.generate-review-card .artifact-review__packet')).toContainText(/approval gate/i);
	await expect(page.locator('[data-testid="generate-review-path"]')).toContainText(/artifact/i);
	await expect(page.locator('[data-testid="generate-review-path"]')).toContainText(/local artifact previews/i);
	...
	const reviewPathLayout = await page.locator('[data-testid="generate-review-path"]').evaluate(element => {
		...
		return {
			...
			allStepsStackDown: stepBoxes.every((box, index) => index === 0 || box.top > stepBoxes[index - 1].top),
			allStepsReadable: stepBoxes.every(box => box.width >= pathBox.width * 0.9),
		};
	});
	expect(reviewPathLayout).toEqual({
		stepCount: 3,
		allStepsStackDown: true,
...
test('Generate page · review packet is visible and opens local artifact drawers', async ({openConsole}) => {
	...
	await reviewCard.getByRole('button', {name: /review pdf artifact/i}).click();
	const drawer = page.getByRole('region', {name: /proposal artifact review drawer/i});
	await expect(drawer).toBeVisible();
	...
	await expect(drawer).toContainText(/acme hvac proposal packet preview/i);
	...
	await expect(drawer).toContainText(/fixture-backed acme hvac review artifact preview/i);
	...
	await expect(drawer).toContainText(/acme hvac source artifact preview/i);
	const sourceJson = drawer.locator('[data-testid="generate-artifact-source-json"]');
...
test('Generate page · artifact query opens the review artifact inside the console', async ({page}) => {
	...
	await expect(drawer).toContainText(/acme hvac proposal packet preview/i);
	await expect(drawer.locator('.workflow-popout__title')).not.toContainText(/proposal draft/i);
	await expect(drawer).toContainText(/preview artifact/i);
	await expect(drawer).toContainText(/sequence_required/i);
	await expect(drawer.locator('iframe[title="Local proposal PDF review preview"]')).toBeVisible();
	await expect(drawer.getByRole('link', {name: /open raw artifact/i})).toHaveCount(0);
});
...
test('Generate page · review path is visible but muted until the draft is ready', async ({openConsole, page}) => {
	...
	const previewButton = reviewCard.getByRole('button', {name: /review pdf artifact/i});
	...
	await expect(reviewCard.locator('[data-testid="generate-review-path"]').getByRole('button', {name: /pdf/i})).toHaveCount(0);
...
test('Generate page · artifact inspection controls stay visible before the checklist', async ({openConsole, page}) => {
	...
	await expect(reviewCard.getByRole('button', {name: /review pdf artifact/i})).toBeVisible();
	await expect(reviewCard.getByRole('button', {name: /inspect source evidence/i})).toBeVisible();
	await expect(reviewPath).toContainText(/local artifact previews/i);
...
test('Generate page · coach launcher moves out of dense artifact controls on short desktop viewports', async ({openConsole, page}) => {
	await page.setViewportSize({width: 1280, height: 640});
	await openConsole();
	await page.locator('.sb__item:has-text("Generate")').first().click();

	await expect(page.locator('html')).toHaveAttribute('data-console-route', 'generate');
	const artifactLinks = page.locator('.artifact-review__links');
	const reviewButton = page.locator('.generate-review-card').getByRole('button', {name: /^review in proposals$/i});
	const coach = page.locator('.coach-launcher');
	await expect(artifactLinks).toBeVisible();
	await expect(coach).toBeVisible();

	const [coachBox, linksBox, reviewBox] = await Promise.all([
		coach.boundingBox(),
		artifactLinks.boundingBox(),
		reviewButton.boundingBox(),
	]);
	expect(coachBox, 'coach launcher should render').not.toBeNull();
	expect(linksBox, 'artifact controls should render').not.toBeNull();
	expect(reviewBox, 'review CTA should render').not.toBeNull();
	expect(coachBox!.y, 'launcher should sit near the shell top on dense Generate').toBeLessThan(140);
	expect(coachBox!.x + coachBox!.width, 'launcher right edge near viewport right').toBeGreaterThan(1280 - 60);
	const overlapsLinks = coachBox!.x < linksBox!.x + linksBox!.width
		&& coachBox!.x + coachBox!.width > linksBox!.x
		&& coachBox!.y < linksBox!.y + linksBox!.height
		&& coachBox!.y + coachBox!.height > linksBox!.y;
	const overlapsReview = coachBox!.x < reviewBox!.x + reviewBox!.width
		&& coachBox!.x + coachBox!.width > reviewBox!.x
		&& coachBox!.y < reviewBox!.y + reviewBox!.height
		&& coachBox!.y + coachBox!.height > reviewBox!.y;
	expect(overlapsLinks, 'coach launcher must not cover artifact controls').toBe(false);
	expect(overlapsReview, 'coach launcher must not cover the review CTA').toBe(false);
});
