...
test.describe('shell', () => {
	...
	test('sidebar keeps ElevenLabs agents prominent and lower tools reachable at short heights', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1366, height: 640});
		await openConsole();

		const initialGeometry = await page.evaluate(() => {
			const footer = document.querySelector('.sb__footer')?.getBoundingClientRect();
			const agentsNav = document.querySelector('.sb__nav[aria-label="ElevenLabs agents"]')?.getBoundingClientRect();
			const toolsNav = document.querySelector('.sb__nav[aria-label="Console tools"]')?.getBoundingClientRect();
			const agentRows = [...document.querySelectorAll('.sb__nav[aria-label="ElevenLabs agents"] .sb__item')]
				.map(element => {
					const rect = element.getBoundingClientRect();
					return {
						bottom: Math.round(rect.bottom),
						label: (element.textContent || '').replaceAll(/\s+/g, ' ').trim(),
						top: Math.round(rect.top),
						visibleAboveFooter: Boolean(footer && rect.top >= 0 && rect.bottom <= footer.top),
					};
				});
			return {
				agentsBeforeTools: Boolean(agentsNav && toolsNav && agentsNav.top < toolsNav.top),
				agentRows,
				footerTop: Math.round(footer?.top || 0),
			};
		});

		expect(initialGeometry.agentsBeforeTools, `agents nav should appear before sidecar tools: ${JSON.stringify(initialGeometry)}`).toBe(true);
		expect(initialGeometry.agentRows.length).toBeGreaterThan(1);
		expect(
			initialGeometry.agentRows.every(row => row.visibleAboveFooter),
			`agent rows should stay visible above the account footer: ${JSON.stringify(initialGeometry)}`,
		).toBe(true);

		await page.locator('.sb__content').evaluate(element => {
			element.scrollTop = element.scrollHeight;
		});
		await page.locator('.sb__nav[aria-label="Console tools"] .sb__item:has-text("Settings")').click();
	...
	test('home breadcrumbs are branding, while child-route breadcrumbs navigate home', async ({openConsole}) => {
		const page = await openConsole();
		await expect(page.getByRole('navigation', {name: /console breadcrumb/i})).toHaveCount(1);
		await expect(page.locator('.tb__crumb--active')).toHaveCount(1);
		await expect(page.locator('.tb__crumbs [aria-current="page"]')).toHaveCount(1);
		await expect(page.locator('.tb__crumb--active')).toHaveText('Mission Control');
		await expect(page.locator('.tb__crumb--active')).toHaveAttribute('aria-current', 'page');
		await expect(page.locator('.tb__crumb--brand')).toHaveText('Wranngle');
		await expect(page.locator('.tb__crumb--workspace')).toHaveText('gtm_ops console');
		await expect(page.locator('.tb__crumb--brand')).toHaveJSProperty('tagName', 'SPAN');
		await expect(page.locator('.tb__crumb--workspace')).toHaveJSProperty('tagName', 'SPAN');
		await expect(page.locator('.tb__crumbs button')).toHaveCount(0);

		await page.locator('.sb__item:has-text("Generate")').first().click();
		await expect(page.locator('.tb__crumb--active')).toHaveCount(1);
		await expect(page.locator('.tb__crumbs [aria-current="page"]')).toHaveCount(1);
		await expect(page.locator('.tb__crumb--active')).toHaveText('Generate Proposal');
		await expect(page.locator('.tb__crumb--brand')).toHaveJSProperty('tagName', 'BUTTON');
		await expect(page.locator('.tb__crumb--workspace')).toHaveJSProperty('tagName', 'BUTTON');
		await expect(page.locator('.tb__crumbs button')).toHaveCount(2);

		await page.locator('.tb__crumb--workspace').click();
		await expect(page.locator('.tb__crumb--active')).toHaveText('Mission Control');
		await expect(page.locator('.tb__crumbs button')).toHaveCount(0);
	});

	test('desktop topbar keeps the active route breadcrumb readable', async ({openConsole, page}) => {
		...
		const geometry = await page.evaluate(() => {
			const active = document.querySelector('.tb__crumb--active');
			const search = document.querySelector('.tb__search');
			const actions = document.querySelector('.tb__actions');
			const rect = (element: Element | null | undefined) => element?.getBoundingClientRect();
			const intersects = (a: DOMRect | undefined, b: DOMRect | undefined) =>
				Boolean(a && b && a.left < b.right && a.right > b.left && a.top < b.bottom && a.bottom > b.top);

			return {
				activeActionsOverlap: intersects(rect(active), rect(actions)),
				activeSearchOverlap: intersects(rect(active), rect(search)),
				clientWidth: active?.clientWidth || 0,
				scrollWidth: active?.scrollWidth || 0,
		...
		expect(
			...
			`active breadcrumb should not ellipsize the Generate route label: ${JSON.stringify(geometry)}`,
		).toBeLessThanOrEqual(geometry.clientWidth + 1);
		expect(geometry.activeSearchOverlap, `active breadcrumb should not paint under search: ${JSON.stringify(geometry)}`).toBe(false);
		expect(geometry.activeActionsOverlap, `active breadcrumb should not paint under actions: ${JSON.stringify(geometry)}`).toBe(false);
	});

	test('tablet topbar keeps full console branding out of the command search hit area', async ({openConsole, page}) => {
		await page.setViewportSize({width: 884, height: 768});
		await openConsole();
		await page.locator('.sb__item:has-text("Generate")').first().click();
		...
		await expect(page.locator('.tb__crumb--workspace')).toHaveText('gtm_ops console');
		await expect(page.locator('.tb__crumb--active')).toHaveText('Generate Proposal');
		await expect(page.locator('.tb__search-placeholder')).toBeHidden();
		await expect(page.locator('.tb__run')).toHaveAccessibleName(/new run/iu);
		...
		const geometry = await page.evaluate(() => {
			...
			const intersects = (a: ReturnType<typeof read>, b: ReturnType<typeof read>) =>
				Boolean(a && b && a.left < b.right && a.right > b.left && a.top < b.bottom && a.bottom > b.top);
			const crumbs = read('.tb__crumbs');
			const active = document.querySelector('.tb__crumb--active');
			const workspace = document.querySelector('.tb__crumb--workspace');
			const suffix = document.querySelector('.tb__crumb--workspace-suffix');
			...
			return {
				...
				crumbs,
				crumbActionsOverlap: intersects(crumbs, actions),
				crumbSearchOverlap: intersects(crumbs, search),
				search,
				searchActionsOverlap: intersects(search, actions),
				workspaceClientWidth: workspace?.clientWidth ?? 0,
				workspaceScrollWidth: workspace?.scrollWidth ?? 0,
				workspaceSuffixDisplay: suffix ? getComputedStyle(suffix).display : null,
			};
		});

		expect(geometry.crumbs, `topbar crumbs should render: ${JSON.stringify(geometry)}`).not.toBeNull();
		expect(geometry.search, `topbar search should render: ${JSON.stringify(geometry)}`).not.toBeNull();
		expect(geometry.actions, `topbar actions should render: ${JSON.stringify(geometry)}`).not.toBeNull();
		expect(geometry.crumbs!.width, `topbar crumbs should reserve visible space: ${JSON.stringify(geometry)}`).toBeGreaterThan(220);
		expect(geometry.workspaceSuffixDisplay, `workspace suffix should stay visible at tablet width: ${JSON.stringify(geometry)}`).not.toBe('none');
		expect(geometry.workspaceScrollWidth, `gtm_ops console should not be visually clipped at tablet width: ${JSON.stringify(geometry)}`).toBeLessThanOrEqual(geometry.workspaceClientWidth + 1);
		expect(geometry.activeScrollWidth, `Generate breadcrumb should stay readable at tablet width: ${JSON.stringify(geometry)}`).toBeLessThanOrEqual(geometry.activeClientWidth + 1);
		expect(geometry.crumbSearchOverlap, `breadcrumb text should not paint under search: ${JSON.stringify(geometry)}`).toBe(false);
		expect(geometry.crumbActionsOverlap, `breadcrumb text should not paint under actions: ${JSON.stringify(geometry)}`).toBe(false);
		expect(geometry.searchActionsOverlap, `search should not paint under actions: ${JSON.stringify(geometry)}`).toBe(false);
	});

	test('desktop-narrow topbar keeps the primary New run action visible, not plus-only', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1040, height: 768});
		await openConsole();
		await page.locator('.sb__item:has-text("Proposals")').first().click();

		const trigger = page.locator('.tb__run');
		await expect(trigger).toBeVisible();
		await expect(trigger.locator('.tb__run-label')).toBeVisible();
		await expect(trigger.locator('.tb__run-label')).toHaveText('New run');

		const geometry = await trigger.evaluate(element => {
			const rect = element.getBoundingClientRect();
			const label = element.querySelector('.tb__run-label');
		...
		expect(geometry.text).toContain('New run');
		expect(geometry.labelDisplay).not.toBe('none');
		expect(geometry.labelWidth, `New run text should be visibly rendered: ${JSON.stringify(geometry)}`).toBeGreaterThan(42);
		expect(geometry.buttonWidth, `New run button should not collapse to the icon-only square: ${JSON.stringify(geometry)}`).toBeGreaterThan(96);
		expect(geometry.overflowing, `New run label should fit inside the button: ${JSON.stringify(geometry)}`).toBe(false);
	});
	...
	test('icon-label buttons keep readable word and icon spacing across dense console surfaces', async ({openConsole}) => {
		...
		await page.locator('.sb__item:has-text("Generate")').first().click();
		await assertReadableButton(page.getByRole('button', {name: /review packet preview/i}), 'Generate review packet preview button');
	...
	test('drawer and admin jumps stay inside console scrollers', async ({openConsole, page}) => {
		...
		await page.getByRole('button', {name: /review pdf preview/i}).click();
	...
	test('sparkline inspection is granular without flooding the tab order', async ({openConsole}) => {
		...
		const scrubber = firstSparkline.locator('[data-testid="sparkline-scrubber"]');
		await expect(scrubber).toBeVisible();
		const latestX = await scrubber.evaluate(node => (node as HTMLElement).getBoundingClientRect().left);
		...
		await expect(firstSparkline.locator('.spark-tooltip')).toHaveCSS('opacity', '1');
		await expect(page.locator('.stat__spark').first()).toHaveCSS('opacity', '1');
		await page.keyboard.press('ArrowLeft');
		await expect(firstSparkline.locator('.spark-tooltip')).toContainText(/ago/i);
		const priorX = await scrubber.evaluate(node => (node as HTMLElement).getBoundingClientRect().left);
		expect(priorX, 'keyboard inspection should move the visible sparkline scrubber to the prior period').toBeLessThan(latestX);
		await page.keyboard.press('Home');
		await expect(firstSparkline.locator('.spark-tooltip')).toContainText(/ago/i);
		const firstX = await scrubber.evaluate(node => (node as HTMLElement).getBoundingClientRect().left);
		expect(firstX, 'Home should pin the scrubber to the first period, not leave a generic tooltip in place').toBeLessThanOrEqual(priorX);
	});
...
test.describe('topbar', () => {
	...
	test('command palette searches proposals and opens the selected proposal review', async ({openConsole}) => {
		...
		if (!target) {
			throw new Error('fixture should include at least one proposal');
		}
	...
	test('"New run" popover layers above the top-right coach launcher', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Agents")').first().click();
		...
		const overlap = await page.evaluate(() => {
			...
			const launcherElement = document.querySelector('.coach-launcher');
			if (!popoverElement || !launcherElement) {
				return {exists: false, owner: ''};
			}

			const popoverRect = popoverElement.getBoundingClientRect();
			const launcherRect = launcherElement.getBoundingClientRect();
			const left = Math.max(popoverRect.left, launcherRect.left);
			const right = Math.min(popoverRect.right, launcherRect.right);
			const top = Math.max(popoverRect.top, launcherRect.top);
			const bottom = Math.min(popoverRect.bottom, launcherRect.bottom);
			...
			return {
				exists: true,
				owner: topElement?.closest('.popover')
					? 'popover'
					: (topElement?.closest('.coach-launcher')
						? 'coach-launcher'
						: topElement?.tagName.toLowerCase() || ''),
			};
		});

		expect(overlap.exists, 'test route should place the coach launcher under the menu area').toBe(true);
		expect(overlap.owner).toBe('popover');

		await popover.locator('.pop__row:has-text("Generate proposal")').click();
	...
	test('command palette result rows are real buttons and hover selects the hovered row', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.tb__search').click();
		const rows = page.locator('.cp__row');
		await expect(rows.first()).toBeVisible();

		const firstFiveTags = await rows.evaluateAll(nodes => nodes.slice(0, 5).map(node => node.tagName));
		expect(firstFiveTags.every(tag => tag === 'BUTTON')).toBe(true);
	...
	test('command palette opens the local eval run plan without claiming execution', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.tb__search').click();
		const cp = page.locator('.cp');
		await cp.locator('input').fill('Open eval run plan');
		await expect(cp.locator('.cp__row:has-text("Trigger eval suite")')).toHaveCount(0);
		await cp.locator('.cp__row:has-text("Open eval run plan")').click();
		...
		expect(ctx.extra.run_intent).toBe('eval_run_plan');
		expect(ctx.extra.eval_harness_command_id).toBe('eval-quick');
		expect(ctx.extra.eval_suite_scope).toBe('quick-domain-batch');
	});
	...
	test('command palette recap action has an operator empty state, not fixture plumbing', async ({openConsole}) => {
		const page = await openConsole();
		await page.evaluate(() => {
			(globalThis as any).GTM.calls = [];
		});

		await page.locator('.tb__search').click();
		const cp = page.locator('.cp');
		await cp.locator('input').fill('draft recap');
		await cp.locator('.cp__row:has-text("Draft recap email")').click();

		const toast = page.locator('.toast').first();
		await expect(toast).toContainText(/no calls to recap/i);
		await expect(toast).toContainText(/record or import a call before drafting a follow-up/i);
		await expect(toast).not.toContainText(/d\.calls|fixture|empty array/i);
	});
	...
	test('notifications footer opens integrations settings instead of a dead tab event', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.tb__bell').click();
		const popover = page.locator('.popover');
		await expect(popover).toBeVisible();
		await expect(popover.getByRole('button', {name: /^mark all read$/i})).toHaveCount(1);

		await popover.getByRole('button', {name: /^settings$/i}).click();

		await expect(page.locator('.popover')).toHaveCount(0);
		await expect(page.locator('.tb__crumb--active')).toContainText('Settings');
		await expect(page.getByRole('tab', {name: /^integrations$/i})).toHaveAttribute('aria-selected', 'true');
		const ctx = await page.evaluate(() => (globalThis as any).AppContext.get());
		expect(ctx.extra.settings_tab).toBe('integrations');
		expect(ctx.extra.triggered_from).toBe('topbar-notifications-footer');
	});
	...
	test('"New run" outbound discovery opens the pipeline intake panel', async ({openConsole}) => {
		...
		const outboundRow = pop.locator('.pop__row').first();
		await expect(outboundRow).toContainText(/opens pipeline lead intake/i);
		await expect(outboundRow).not.toContainText(/agent-\d+|hunter|toast/i);
	...
	test('"New run" scoring action opens a review view instead of pretending to re-score', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.tb .btn--primary:has-text("New run")').click();
		const pop = page.locator('.popover');
		const scoringRow = pop.locator('.pop__row:has-text("Review stale lead scores")');
		await expect(scoringRow).toBeVisible();
		await expect(scoringRow).toContainText(/opens pipeline scoring saved view/i);
		await expect(pop.locator('.pop__row:has-text("Re-score stale leads")')).toHaveCount(0);
		await scoringRow.click();

		await expect(page.locator('.tb__crumb--active')).toContainText('Pipeline');
		await expect(page.locator('.workflow-popout:has-text("Pipeline filters")')).toBeVisible();
		const ctx = await page.evaluate(() => (globalThis as any).AppContext.get());
		expect(ctx.extra.triggered_from).toBe('topbar-new-run');
		expect(ctx.extra.run_intent).toBe('lead_rescore');
		expect(ctx.extra.pipeline_panel).toBe('filters');
		expect(ctx.extra.pipeline_filter).toBe('all');
	});
	...
	test('"New run" Generate proposal carries call context into the review draft composer', async ({openConsole}) => {
		...
		const pop = page.locator('.popover');
		await expect(pop).toBeVisible();
		const generateRow = pop.locator('.pop__row:has-text("Generate proposal")');
		await expect(generateRow).toContainText(new RegExp(sourceCall.id));
		await expect(generateRow).toContainText(/buyer proof/i);
		await generateRow.click();
		...
		const ctx = await page.evaluate(() => (globalThis as any).AppContext.get());
		expect(ctx.selection).toEqual({type: 'call', id: sourceCall.id});
	...
	test('Generate review path exposes explicit artifact and proposal navigation', async ({openConsole}) => {
		...
		const artifactStep = page.locator('[data-testid="generate-review-path-step-artifact"]');
		await expect(artifactStep).toContainText(/review the sample packet/i);
		await expect(artifactStep).not.toContainText(/open/i);
		const initialArtifactAction = page.locator('[data-testid="generate-review-path-action-review-artifact-preview"]');
		await expect(initialArtifactAction).toBeVisible();
		await expect(initialArtifactAction).toHaveAccessibleName(/path shortcut: review sample packet/i);
		await expect(initialArtifactAction).toHaveText(/review sample packet/i);
		await expect(initialArtifactAction).not.toHaveText(/open/i);
		await page.getByRole('button', {name: /use sample buyer proof/i}).first().click();
		await expect(artifactStep).toContainText(/review packet preview only/i);
		await expect(artifactStep).not.toContainText(/open/i);
		const previewArtifactAction = page.locator('[data-testid="generate-review-path-action-review-artifact-preview"]');
		await expect(previewArtifactAction).toBeVisible();
		await expect(previewArtifactAction).toHaveAccessibleName(/path shortcut: inspect packet preview/i);
		await expect(previewArtifactAction).toHaveText(/inspect packet preview/i);
		await expect(previewArtifactAction).not.toHaveText(/open/i);
		await previewArtifactAction.click();
		...
		const draftArtifactAction = page.locator('[data-testid="generate-review-path-action-review-artifact-draft"]');
		await expect(draftArtifactAction).toHaveAccessibleName(/path shortcut: review draft pdf/i);
		await expect(draftArtifactAction).toHaveText(/review draft pdf/i);
		await expect(draftArtifactAction).not.toHaveText(/open/i);
		await page.locator('[data-testid="generate-review-path-action-open-proposals-review"]').click();
		await expect(page.locator('.tb__crumb--active')).toContainText('Proposals');
	});

	test('"New run" eval action opens the in-console run plan without claiming execution', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.tb .btn--primary:has-text("New run")').click();
		const pop = page.locator('.popover');
		await expect(pop.locator('.pop__row:has-text("Trigger eval suite")')).toHaveCount(0);
		const evalRow = pop.locator('.pop__row:has-text("Open eval run plan")');
		await expect(evalRow).toContainText(/opens local eval run plan/i);
		await expect(evalRow).not.toContainText(/cmd\+e|queued|trigger/i);
		await evalRow.click();
		await expect(page.locator('.tb__crumb--active')).toContainText('Evals');
		await expect(page.locator('.eval-bridge-popout')).toBeVisible();
		await expect(page.getByTestId('eval-run-plan-summary')).toContainText(/console eval manifest\s*·\s*8 commands/i);
		await expect(page.getByTestId('eval-harness-command-detail')).toContainText(/quick gtm eval batch|bun run eval:quick/i);
		await expect(page.getByTestId('eval-run-plan-summary')).not.toContainText(/eval-harness\.manifest\.json|voice_ai_agent_evals/i);
		const ctx = await page.evaluate(() => (globalThis as any).AppContext.get());
		expect(ctx.extra.triggered_from).toBe('topbar-new-run');
		expect(ctx.extra.run_intent).toBe('eval_run_plan');
		expect(ctx.extra.eval_suite_scope).toBe('quick-domain-batch');
...
test.describe('charts', () => {
	test('sparkline hover and keyboard inspection expose the exact data point, not a generic trend tooltip', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1280, height: 900});
		await openConsole();

		const spark = page.locator('.stat__spark .spark-wrap').first();
		await expect(spark).toBeVisible();
		const box = await spark.boundingBox();
		expect(box, 'mission-control sparkline should render').not.toBeNull();
		if (!box) {
			return;
		}
		...
		expect(firstLabel).toMatch(/pipeline trend/i);
		expect(firstLabel).toMatch(/baseline/i);
		expect(firstLabel).toMatch(/point 1\/12/i);
		await expect(spark).toHaveAttribute('aria-label', /current \$1\.87m, \+12\.4% vs last week/i);

		await page.mouse.move(box.x + box.width - 2, box.y + box.height / 2);
		await expect(tooltip).toContainText(/latest/i);
		await expect(tooltip).toContainText(/point 12\/12/i);
		const latestLabel = (await tooltip.textContent())?.trim() || '';
		expect(latestLabel).not.toEqual(firstLabel);
		await expect(spark).toHaveAttribute('data-active-label', latestLabel);
...
test.describe('mission control', () => {
	...
	test('attention banner "Review now" opens the flagged call review panel', async ({openConsole}) => {
		...
		await expect(panel.getByRole('button', {name: /generate revised proposal/i})).toBeVisible();
		await expect(panel).not.toContainText(/proposal v\d+/i);
	...
	test('history refresh preserves curated pipeline leads beside generated-run leads', async ({openConsole}) => {
		const page = await openConsole();

		await expect.poll(async () => page.evaluate(() => {
			const ids = new Set(((globalThis as any).GTM.companies || []).map((c: any) => c.id));
			return {
				hasCuratedLead: ids.has('banyan'),
				hasGeneratedRunLead: ids.has('acme-hvac-r3'),
				total: ids.size,
			};
		})).toMatchObject({
			hasCuratedLead: true,
			hasGeneratedRunLead: true,
		});
	});

	test('hot lead arrow opens the selected lead in Pipeline instead of only changing routes', async ({openConsole}) => {
		const page = await openConsole();
		await expect.poll(async () => page.evaluate(() => {
			const companies = [...((globalThis as any).GTM.companies || [])]
				.sort((a: any, b: any) => Number(b.score || 0) - Number(a.score || 0));
			const openButton = document.querySelector<HTMLElement>('[data-testid="hot-lead-open"]');
			const buttonId = openButton?.dataset.companyId || '';
			return Boolean(companies[0]?.id && buttonId === companies[0].id);
		})).toBe(true);
		const expected = await page.evaluate(() => {
			const companies = [...((globalThis as any).GTM.companies || [])]
				.sort((a: any, b: any) => Number(b.score || 0) - Number(a.score || 0));
		...
		const openButton = page.locator('[data-testid="hot-lead-open"]').first();
		await expect(openButton).toBeVisible();
		await expect(openButton).toHaveAttribute('data-company-id', expected.id);
		await openButton.click();

		await expect(page.locator('.tb__crumb--active')).toContainText('Pipeline');
		const ctx = await page.evaluate(() => (globalThis as any).AppContext.get());
		expect(ctx.selection).toEqual({type: 'lead', id: expected.id});
		expect(ctx.extra.triggered_from).toBe('mission-hot-lead');
		await expect(page.locator(`[data-testid="pipe-card"][data-company-id="${expected.id}"]`)).toHaveAttribute('data-selected', 'true');
		await expect(page.getByRole('dialog', {name: /lead detail/i})).toContainText(expected.name);
	});

	test('Mission Control header sub is derived from live state - agent status and attention pressure update with the data', async ({openConsole}) => {
		const page = await openConsole();
		...
		// Pattern: "<N> agent(s) · <A> active agent(s) / <P> paused · ... attention".
		// The fixture's task counter is workload volume; it should not be
		// promoted as current operator pressure in the page headline.
		const subText = (await sub.textContent() || '').trim();
		expect(subText).toMatch(/\d+ agents?/i);
		expect(subText).toMatch(/\d+ active agents? \/ \d+ paused/i);
		expect(subText).not.toMatch(/\d+ assigned tasks?/i);
		expect(subText).not.toMatch(/\d+ open tasks?/i);
		expect(subText).toMatch(/(review items? needs? attention|all attention items snoozed)/i);

		// Reading the live counts from the GTM fixture confirms the headline math.
		const live = await page.evaluate(() => {
			const D = (globalThis as any).GTM;
			const agentCount = (D.agents || []).length;
			const activeAgents = (D.agents || []).filter((a: any) => a.status === 'active').length;
			const pausedAgents = (D.agents || []).filter((a: any) => a.status === 'paused').length;
			const assignedTasks = (D.agents || []).reduce((s: number, a: any) => s + (Number(a.tasks) || 0), 0);
			return {
				activeAgents, agentCount, assignedTasks, pausedAgents,
			};
		});
		expect(subText).toContain(`${live.agentCount} agent`);
		expect(subText).toContain(`${live.activeAgents} active agent`);
		expect(subText).toContain(`${live.pausedAgents} paused`);
		expect(subText).not.toContain(`${live.assignedTasks} assigned task`);
		...
		await expect(sub).toContainText(/review items? needs? attention/i);
	});
	...
	test('Mission Control opens the eval run plan without claiming to run every eval', async ({openConsole}) => {
		const page = await openConsole();
		await expect(page.getByRole('button', {name: /run all evals/i})).toHaveCount(0);
		await page.getByRole('button', {name: /open eval run plan/i}).click();
		...
		const ctx = await page.evaluate(() => (globalThis as any).AppContext.get());
		expect(ctx.extra.triggered_from).toBe('mission-open-eval-run-plan');
		expect(ctx.extra.run_intent).toBe('eval_run_plan');
		expect(ctx.extra.eval_harness_command_id).toBe('eval-quick');
		expect(ctx.extra.eval_suite_scope).toBe('quick-domain-batch');
	});
	...
	test('Pause queue / Resume all toggle queue state and toast — toast facts derive from live agents, not "throttled 80%"', async ({openConsole}) => {
		...
		const live = await page.evaluate(() => {
			...
			const assigned = agents.reduce((s, a) => s + (Number(a.tasks) || 0), 0);
			const avgSuccess = agents.length > 0
				? Math.round(agents.reduce((s, a) => s + (Number(a.success) || 0), 0) / agents.length * 100)
				: 0;
			return {count: agents.length, assigned, avgSuccess};
		});

		await pause.click();
		await expect(page.locator('.toast').first()).toContainText(/queue paused/i);
		await expect(page.locator('.toast', {hasText: new RegExp(`${live.assigned} assigned task`)})).toBeVisible();
		await expect(page.locator('.toast', {hasText: /\d+ in-flight task/i})).toHaveCount(0);
	...
	test('sparkline metrics expose a hover popout', async ({openConsole}) => {
		...
		await expect(spark.locator('.spark-tooltip')).toHaveAttribute('data-edge', 'start');
...
test.describe('pipeline', () => {
	...
	test('Pipeline board fits all five stages at standard desktop width', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1366, height: 768});
		await openConsole();
		await page.locator('.sb__item:has-text("Pipeline")').first().click();
		await expect(page.locator('.pipe__col')).toHaveCount(5);
		...
		const geometry = await page.evaluate(() => {
			...
			const columns = [...pipe.querySelectorAll<HTMLElement>('.pipe__col')].map(column => {
				const box = column.getBoundingClientRect();
				return {
					left: box.left,
					right: box.right,
					stage: column.dataset.stageId || '',
					width: box.width,
				};
			});
			...
			return {
				...
				gridColumns: getComputedStyle(pipe).gridTemplateColumns.trim().split(/\s+/u),
				pipeClientWidth: pipe.clientWidth,
				pipeLeft: pipeBox.left,
				pipeRight: pipeBox.right,
				pipeScrollWidth: pipe.scrollWidth,
				viewportWidth: window.innerWidth,
			};
		});

		expect(geometry, 'Pipeline board should render').not.toBeNull();
		expect(geometry!.columnCount).toBe(5);
		expect(geometry!.gridColumns).toHaveLength(5);
		expect(
			geometry!.pipeScrollWidth,
			`Pipeline board should not require horizontal scrolling at 1366px: ${JSON.stringify(geometry)}`,
		).toBeLessThanOrEqual(geometry!.pipeClientWidth + 1);
		expect(
			geometry!.columns.every(column => column.width >= 190),
			`Pipeline columns should stay readable while fitting the work surface: ${JSON.stringify(geometry)}`,
		).toBe(true);
		expect(
			geometry!.columns.every(column => column.left >= geometry!.pipeLeft - 1 && column.right <= geometry!.pipeRight + 1),
			`Every Pipeline stage column should be visible inside the board: ${JSON.stringify(geometry)}`,
		).toBe(true);
		expect(
			geometry!.docScrollWidth,
			`Pipeline route should not create page-level horizontal scroll: ${JSON.stringify(geometry)}`,
	...
	test('lead artifacts open in-console instead of a raw fixture tab', async ({openConsole}) => {
		...
		const trigger = panel.getByRole('button', {name: /review artifacts/i});
		await expect(trigger).toHaveAttribute('aria-controls', 'lead-artifact-review-drawer');
		...
		await expect(drawer).toContainText(/json review preview/i);
		await expect(drawer).toContainText(/source evidence record/i);
		await expect(drawer).toContainText(/output\/acme-hvac\/r3\/schema\.json/);
		await expect(drawer).toContainText(/review preview/i);
		await expect(drawer).not.toContainText(/public fixture|review packet path|path only/i);
		await expect(drawer.locator('pre')).toContainText(/tr_demo_acme_001/);
		await drawer.getByRole('button', {name: /copy source reference/i}).click();
		await expect(page.locator('.toast', {hasText: /source reference/i})).toBeVisible();
		await expect(page.locator('.toast', {hasText: /artifact path/i})).toHaveCount(0);
	...
	test('lead detail recent activity names proposal review packets instead of internal versions', async ({openConsole}) => {
		const page = await openConsole();
		await page.waitForFunction(
			() => (globalThis as any).GTM?.companies?.some((c: any) => c.id === 'acme-hvac-r3'),
			null,
		...
		const panel = page.locator('[role="dialog"][aria-label^="Lead detail"]');
		await expect(panel).toBeVisible();
		await expect(panel).toContainText(/proposal review packet dispatched/i);
		await expect(panel).not.toContainText(/proposal v\d+/i);
	});
...
test.describe('calls', () => {
	...
	test('Calls active-call header wraps actions without crushing buyer context or page width', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1440, height: 900});
		await openConsole();
		await page.locator('.sb__item:has-text("Calls")').first().click();
		await expect(page.locator('.calls-grid__transcript .card__title')).toContainText(/call-2419/i);

		const layout = await page.evaluate(() => {
			const scroll = document.querySelector('.scroll');
			const title = document.querySelector('.calls-grid__transcript .card__title');
			const action = document.querySelector('.calls-grid__transcript .card__hd > :not(.card__title)');
			const titleBox = title?.getBoundingClientRect();
			const actionBox = action?.getBoundingClientRect();
			return {
				actionTop: actionBox?.top ?? 0,
				pageClientWidth: scroll?.clientWidth ?? 0,
				pageScrollWidth: scroll?.scrollWidth ?? 0,
				titleHeight: titleBox?.height ?? 0,
				titleText: (title?.textContent || '').replaceAll(/\s+/g, ' ').trim(),
				titleTop: titleBox?.top ?? 0,
				titleWidth: titleBox?.width ?? 0,
			};
		});

		expect(layout.titleText, `active call title should preserve buyer/contact context: ${JSON.stringify(layout)}`).toMatch(/CALL-2419 .* Banyan Health .* Priya Mendel/);
		expect(layout.titleWidth, `active call title should not collapse into a one-word column: ${JSON.stringify(layout)}`).toBeGreaterThanOrEqual(260);
		expect(layout.titleHeight, `active call title should be compact, not vertical text: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(56);
		expect(layout.actionTop, `header actions should wrap below the call title when needed: ${JSON.stringify(layout)}`).toBeGreaterThanOrEqual(layout.titleTop);
		expect(layout.pageScrollWidth, `Calls route should not create page-level horizontal scroll: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(layout.pageClientWidth + 1);
	});
...
test.describe('proposals · seed preservation', () => {
	test('history-derived proposals merge with the curated data.js seeds without dropping them or showing slug-shaped company names', async ({openConsole}) => {
		...
		const acmeRow = list.locator('[data-testid="proposal-row"]').filter({hasText: 'Acme HVAC Services'}).first();
		await expect(acmeRow).toBeVisible({timeout: 10_000});
		await expect(acmeRow).toContainText(/generated .*review artifacts/i);
		await expect(acmeRow).not.toContainText(/viewed today/i);
		await expect(acmeRow).not.toContainText(/sent 5\/\d\/2024/i);
		...
		// company display. They should also read as local review artifacts,
		// not buyer-sent documents with fake "viewed today" telemetry.
...
test.describe('proposals · sections card', () => {
	...
	test('proposal blocker CTA names the Generate handoff and gets a full-width action row', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1280, height: 720});
		await openConsole();
		await page.locator('.sb__item:has-text("Proposals")').first().click();
		await page.locator('[data-testid="proposal-row"]').filter({hasText: 'PR-2041'}).first().click();

		const detail = page.locator('.split--2 > .vstack > .card').first();
		const cta = detail.getByTestId('proposal-address-blockers');
		await expect(cta).toHaveText(/address blockers · 2/i);
		await expect(cta).toHaveAttribute('title', /open generate with 2 blockers carried into a revised review packet/i);

		const layout = await detail.evaluate(card => {
			const actions = card.querySelector<HTMLElement>('.proposal-detail-actions');
			const buttons = [...card.querySelectorAll<HTMLElement>('.proposal-detail-actions .btn')];
			const blocker = card.querySelector<HTMLElement>('[data-testid="proposal-address-blockers"]');
			...
			const rows = buttons.map(read);
			const blockerBox = read(blocker);
			const peerRows = rows.filter(row => row && row.text !== blockerBox?.text);
			return {
		...
		expect(layout.actions, `proposal detail actions should render: ${JSON.stringify(layout)}`).not.toBeNull();
		expect(layout.blocker, `blocker CTA should render: ${JSON.stringify(layout)}`).not.toBeNull();
		expect(layout.blocker!.scrollWidth, `blocker CTA text should fit without clipping: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(layout.blocker!.clientWidth + 1);
		expect(layout.blocker!.width, `Generate handoff CTA should use the full action row: ${JSON.stringify(layout)}`).toBeGreaterThan((layout.actions!.width * 0.9));
		expect(layout.peerRows.every(row => row && row.bottom <= layout.blocker!.top), `review/send actions should sit above the Generate handoff row: ${JSON.stringify(layout)}`).toBe(true);
		expect(layout.blocker!.height, `Generate handoff CTA should not become a cramped wrapped chip: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(52);
	});

	test('coach launcher stays visible on Proposals without covering the workbench', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1280, height: 720});
		await openConsole();
		await page.locator('.sb__item:has-text("Proposals")').first().click();
		...
		const layout = await page.evaluate(() => {
			...
			const launcherBox = read('.coach-launcher');
			const launcherLabel = document.querySelector('.coach-launcher__label');
			return {
				labelDisplay: launcherLabel ? getComputedStyle(launcherLabel).display : null,
				launcher: launcherBox,
				overlapsDetailActions: overlaps(launcherBox, read('.proposal-detail-actions')),
				overlapsHeaderActions: overlaps(launcherBox, read('.ph__actions')),
				overlapsList: overlaps(launcherBox, read('.proposals-list-card')),
				rightOffset: launcherBox ? window.innerWidth - launcherBox.right : null,
			};
		});

		expect(layout.launcher, `coach launcher should render: ${JSON.stringify(layout)}`).not.toBeNull();
		expect(layout.labelDisplay, 'dense Proposals route should show the compact orb, not a full pill').toBe('none');
		expect(layout.launcher!.top, 'launcher should sit in the top route gutter below the topbar').toBeGreaterThanOrEqual(56);
		expect(layout.launcher!.top, 'launcher should not drift into the proposal rows').toBeLessThan(120);
		expect(layout.rightOffset, 'launcher right edge should stay near the viewport edge').toBeLessThanOrEqual(40);
		expect(layout.overlapsHeaderActions, `launcher should not cover proposal filters/actions: ${JSON.stringify(layout)}`).toBe(false);
		expect(layout.overlapsList, `launcher should not cover proposal rows: ${JSON.stringify(layout)}`).toBe(false);
		expect(layout.overlapsDetailActions, `launcher should not cover proposal detail actions: ${JSON.stringify(layout)}`).toBe(false);
	});
});

test.describe('proposals · resend form', () => {
	test('blocked proposals open a send-hold gate instead of a buyer SEND form', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Proposals")').first().click();

		const target = await page.evaluate(() => {
			const proposals = ((globalThis as any).GTM.proposals || []) as Array<{id: string; blockers?: string[]}>;
			return proposals.find(p => Array.isArray(p.blockers) && p.blockers.length > 0)?.id || null;
		});
		if (!target) {
			return;
		}

		await page.locator('[data-testid="proposal-row"]').filter({hasText: target}).click();
		const sendGate = page.locator('[data-testid="proposal-resend-open"]');
		await expect(sendGate).toHaveAttribute('data-send-gate', 'blocked');
		await expect(sendGate).toContainText(/send held/i);
		await expect(page.getByTestId('proposal-send-gate-note')).toContainText(/buyer send is held/i);

		await sendGate.click();
		const hold = page.getByTestId('proposal-send-hold');
		await expect(hold).toBeVisible();
		await expect(hold).toContainText(/buyer send held/i);
		await expect(hold).toContainText(/draft a revised review packet in generate/i);
		expect(await hold.getByTestId('proposal-send-hold-blocker').count()).toBeGreaterThan(0);
		await expect(page.locator('[data-testid="proposal-resend-form"]')).toHaveCount(0);
		await expect(page.locator('[data-testid="proposal-resend-send"]')).toHaveCount(0);

		await page.getByTestId('proposal-send-hold-address-blockers').click();
		await expect(page.locator('.tb__crumb--active')).toContainText('Generate');
		await expect(page.locator('[data-testid="generate-address-blockers-banner"]')).toContainText(new RegExp(`Addressing blockers from ${target}`));
	});
...
test.describe('proposals', () => {
	...
	test('"Review packet" proposal opens a selectable local artifact viewer', async ({openConsole}) => {
		...
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').first()).toHaveAttribute('data-preview-state', 'available');
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').first()).toContainText(/review sample pdf/i);
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').first()).toContainText(/sample pdf preview attached for local review/i);
		await expect(panel.locator('[data-testid="proposal-review-artifact-preview"] iframe')).toHaveAttribute('src', '../assets/sample-proposal.pdf');
		await expect(panel.getByTestId('proposal-review-preview-unavailable')).toHaveCount(0);

		await panel.getByRole('button', {name: /missing source evidence artifact/i}).click();
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').nth(1)).toHaveAttribute('data-active', 'true');
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').nth(1)).toHaveAttribute('data-preview-state', 'unavailable');
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').nth(1)).toContainText(/not attached/i);
		await expect(panel.getByTestId('proposal-review-preview-unavailable')).toContainText(/source evidence preview unavailable/i);
		await expect(panel.getByTestId('proposal-review-preview-unavailable')).toContainText(/source evidence is not attached to this review packet yet/i);
		await expect(panel.getByTestId('proposal-review-preview-unavailable')).toContainText(/review\/pr-2041\/source-evidence\.json/i);
		await expect(panel.locator('[data-testid="proposal-review-artifact-preview"] iframe')).toHaveCount(0);
	});

	test('"Review packet" uses a framed PDF preview and attached source artifact previews', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Proposals")').first().click();
		const historyProposal = page.locator('[data-testid="proposal-row"]').filter({hasText: 'acme-hvac-r3'}).first();
		await expect(historyProposal).toBeVisible({timeout: 10_000});
		await historyProposal.click();
		...
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').first()).toContainText(/review sample pdf/i);
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').first()).toContainText(/sample pdf preview attached for local review/i);
		await expect(panel.locator('iframe[title="Acme HVAC Services sample PDF review preview"]')).toBeVisible();

		await panel.getByRole('button', {name: /review source evidence artifact/i}).click();
		await expect(panel.locator('[data-testid="proposal-review-artifact"]').nth(1)).toHaveAttribute('data-preview-state', 'available');
		await expect(panel.locator('[data-testid="proposal-review-source-json"]')).toContainText(/tr_demo_acme_001/i);
		await expect(panel.getByTestId('proposal-review-preview-unavailable')).toHaveCount(0);
		await expect(panel.locator('[data-testid="proposal-review-artifact-preview"] iframe')).toHaveCount(0);
	});
});
...
test.describe('evals', () => {
	test('loads harness runs, axis detail, and ElevenLabs lab surface', async ({openConsole}) => {
		...
		await expect(page.locator('[data-testid="eval-run-plan-summary"]')).toContainText(/quick gtm eval batch ready/i);
		await expect(page.locator('[data-testid="eval-run-plan-summary"]')).toHaveAttribute('data-state', 'ready');
	...
	test('header separates loaded eval results from suite-library run history', async ({openConsole}) => {
		...
		await expect(page.getByTestId('eval-masthead-results')).toHaveText(/8 loaded results/i);
		await expect(page.getByTestId('eval-masthead-suite-runs')).toHaveText(/7,690 suite-library runs/i);
		await expect(page.getByTestId('eval-masthead-results')).not.toContainText(/suite-library/i);
	});

	test('mobile Evals masthead splits run facts into unclipped chips', async ({openConsole, page}) => {
		await page.setViewportSize({width: 390, height: 844});
		await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		await expect(page.locator('.eval-run-row')).not.toHaveCount(0, {timeout: 5000});

		const chips = page.locator('[data-testid="eval-console-masthead"] .eval-command-center__route-stat');
		await expect(chips).toHaveCount(3);
		await expect(page.getByTestId('eval-masthead-results')).toHaveText(/8 loaded results/i);
		await expect(page.getByTestId('eval-masthead-suite-runs')).toHaveText(/7,690 suite-library runs/i);

		const layout = await chips.evaluateAll(elements => elements.map(element => {
		...
		for (const chip of layout) {
			expect(chip.clipped, `${chip.text} should not be clipped: ${JSON.stringify(layout)}`).toBe(false);
			expect(chip.width, `${chip.text} should have measurable width: ${JSON.stringify(layout)}`).toBeGreaterThan(24);
			expect(chip.height, `${chip.text} should stay chip-scale: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(34);
		}
	});

	test('Evals masthead keeps the local ElevenLabs wrapper fact as one readable chip', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1366, height: 768});
		await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		await expect(page.locator('.eval-run-row')).not.toHaveCount(0, {timeout: 5000});

		const agentsChip = page.getByTestId('eval-masthead-agents');
		await expect(agentsChip).toContainText(/\d+\s+elevenlabs agents?/i);
		const geometry = await agentsChip.evaluate(element => {
			const rect = element.getBoundingClientRect();
			const range = document.createRange();
			range.selectNodeContents(element);
			const textRows = range.getClientRects().length;
			range.detach();
			const style = getComputedStyle(element as HTMLElement);
			return {
				height: Math.round(rect.height),
		...
		expect(geometry.textRows, `ElevenLabs agent masthead fact should not split into a dangling word: ${JSON.stringify(geometry)}`).toBe(1);
		expect(geometry.scrollWidth, `ElevenLabs agent masthead fact should fit inside its chip: ${JSON.stringify(geometry)}`).toBeLessThanOrEqual(geometry.width + 1);
		expect(geometry.height, `masthead chip should stay compact: ${JSON.stringify(geometry)}`).toBeLessThanOrEqual(20);
	});

	test('harness failures action is a real toggle, not toast feedback', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		await expect(page.locator('.eval-run-row')).not.toHaveCount(0, {timeout: 5000});

		const filter = page.getByTestId('eval-runs-failures-filter');
		const allCount = await page.locator('.eval-run-row').count();
		await expect(filter).toHaveAttribute('aria-pressed', 'false');

		await filter.click();
		await expect(filter).toHaveAttribute('aria-pressed', 'true');
		await expect(filter).toHaveText(/all runs/i);
		await expect(page.locator('.toast')).toHaveCount(0);

		const visibleVerdicts = await page.locator('.eval-run-row').evaluateAll(rows =>
			rows.map(row => (row as HTMLElement).dataset.verdict));
		expect(visibleVerdicts.length, 'fixture should have at least one failing run').toBeGreaterThan(0);
		expect(visibleVerdicts.every(verdict => verdict === 'fail')).toBe(true);
		expect(visibleVerdicts.length, 'failures toggle should narrow the harness list').toBeLessThan(allCount);

		await filter.click();
		await expect(filter).toHaveAttribute('aria-pressed', 'false');
		await expect(filter).toHaveText(/failures/i);
		await expect(page.locator('.eval-run-row')).toHaveCount(allCount);
	});

	test('selected harness run carries its suite family into the suite trend panel', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		await expect(page.locator('.eval-run-row')).not.toHaveCount(0, {timeout: 5000});

		await page.locator('.eval-run-row', {hasText: /knowledge base pricing question/i}).click();
		await expect(page.locator('[data-testid="eval-active-scenario-title"]')).toContainText('Knowledge Base Pricing Question');
		await expect(page.locator('[data-testid="eval-active-suite-id"]')).toContainText('suite objection-pricing');
		await expect(page.locator('.card__title:has-text("suite family · objection-pricing")')).toBeVisible();
		await expect(page.getByTestId('eval-suite-context-note')).toContainText(/Knowledge Base Pricing Question/);
		await expect(page.getByTestId('eval-suite-context-note')).toContainText(/Objection — Pricing Pushback · objection-pricing/);
	});
	...
	test('uses bundled eval artifacts when /api/eval-runs is unavailable', async ({page}) => {
		...
		await expect(page.locator('.eval-run-row').first()).toContainText(/active prompt profile/i);
		await expect(page.locator('.eval-axis-row')).not.toHaveCount(0);
	});
	...
	test('Evals run-bound lab actions stay disabled until a harness run is loaded', async ({openConsole, page}) => {
		...
		const localAdmin = page.locator('[data-testid="eval-command-open-admin"]');
		const syncEvidence = page.locator('[data-testid="eval-sync-context-evidence"]');
		const artifacts = page.locator('[data-testid="eval-command-open-artifact"]');
		...
		await expect(artifacts).toHaveAttribute('title', /load a harness run before opening artifacts/i);
		await expect(page.locator('[data-testid="eval-artifact-panel"]')).toHaveCount(0);
		...
		await expect(artifacts).toHaveAttribute('title', /open .* evidence artifact/i);
	...
	test('Evals local agent admin carries the active run into the Agents context panel', async ({openConsole}) => {
		...
		await expect(readiness).toContainText(/active prompt profile/i);
		await expect(readiness).toContainText(/ready for local admin review and evidence sync/i);
		const commandAdmin = page.getByTestId('eval-command-center').getByTestId('eval-command-open-admin');
		await expect(commandAdmin).toBeVisible();
		await expect(commandAdmin).toBeInViewport();

		await commandAdmin.click();
		...
		await expect(topHandoff).toContainText(/active prompt profile/i);
		...
		await expect(topHandoff).toContainText(/local review packet/i);
		await expect(topHandoff).not.toContainText(/\.\.\/fixtures|fixtures\/runs|\.json/i);
		await expect(topHandoff).not.toContainText(/asr_robustness/i);
		await expect(topHandoff.getByRole('button', {name: /review evidence/i})).toBeVisible();
		...
		await expect(handoff).toContainText(/active prompt profile/i);
		...
		await expect(handoff).toContainText(/local review packet/i);
		await expect(handoff).not.toContainText(/\.\.\/fixtures|fixtures\/runs|\.json/i);
		await expect(handoff).not.toContainText(/asr_robustness/i);
		await expect(handoff.getByRole('button', {name: /review evidence/i})).toBeVisible();
		...
		expect(ctx.extra.selected_eval_context).toContain('active prompt profile');
		expect(ctx.extra.selected_eval_context).not.toContain('prompt/sewy/v');
		...
		expect(ctx.extra.eval_run.prompt_tag).toContain('prompt/sewy/v');
	...
	test('Evals local agent admin Review evidence returns to the in-console artifact panel', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		await expect(page.locator('.eval-run-row')).not.toHaveCount(0, {timeout: 5000});
		const scenarioText = await page.locator('[data-testid="eval-active-scenario-id"]').textContent();
		const scenario = (scenarioText || '').replace(/^scenario\s+/i, '').trim();
		await page.getByTestId('eval-command-center').getByTestId('eval-command-open-admin').click();
		await expect(page.locator('.tb__crumb--active')).toContainText('Agents');
		const handoff = page.getByTestId('agent-eval-handoff-banner');
		await expect(handoff).toContainText(/local review packet/i);
		await expect(handoff).not.toContainText(/\.\.\/fixtures|fixtures\/runs|\.json/i);

		await handoff.getByRole('button', {name: /review evidence/i}).click();
	...
	test('Evals local agent admin replaces any sealed Agents context snapshot', async ({openConsole}) => {
		...
		await page.locator('[data-testid="eval-command-open-admin"]').click();
	...
	test('Workspace Agents navigation opens the general playground, not stale eval admin context', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		await expect(page.locator('.eval-run-row')).not.toHaveCount(0, {timeout: 5000});
		await page.locator('[data-testid="eval-command-open-admin"]').click();
		...
		await page.locator('.sb__nav[aria-label="Workspace navigation"] .sb__item:has-text("Agents")').click();
		...
		expect(ctx.extra.triggered_from).toBe('agents-workspace-nav');
		expect(ctx.extra.eval_run).toBeUndefined();
		expect(ctx.extra.selected_eval_run).toBeUndefined();
		expect(ctx.extra.eval_admin_return_route).toBeUndefined();
		expect(ctx.extra.agent_admin_panel).toBeUndefined();
	...
	test('Evals artifacts action opens a focused in-viewport artifact drawer', async ({openConsole, page}) => {
		...
		await expect(drawer).toBeVisible();
		await expect(drawer).toBeFocused();
		await expect(drawer).toContainText(/evidence artifact/i);
		...
		await expect(drawer).toContainText(/evidence artifact/i);
		await expect(drawer).not.toContainText(/local path/i);
		await expect(drawer.locator('[data-testid="eval-artifact-path"]')).toContainText(/source evidence|local eval artifact index/i);
		await expect(drawer.locator('[data-testid="eval-artifact-path"]')).not.toContainText(/fixtures\/runs|eval-runs\.json/i);
		expect((await drawer.locator('[data-testid="eval-artifact-path"]').innerText()).trim()).not.toMatch(/^\.\.\//);
	...
	test('active ElevenLabs binding metadata wraps instead of clipping in the native Evals header', async ({openConsole, page}) => {
		...
		await expect(voiceMeta).toContainText(/Sales Coach · local eval wrapper/);
		await expect(voiceMeta).toContainText(/Agents \/ sales_coach/);

		const layout = await voiceMeta.evaluate(node =>
			[...node.querySelectorAll('.eval-command-center__voice-fact strong')].map(value => {
		...
		expect(layout, 'agent and local wrapper values should render').toHaveLength(2);
		for (const value of layout) {
			expect(value.clipped, `${value.text} should not be horizontally clipped`).toBe(false);
			expect(value.whiteSpace, `${value.text} should be allowed to wrap`).toBe('normal');
		}

		expect(layout.some(value => value.overflowWrap === 'anywhere')).toBe(true);
	});

	test('Evals pass-rate sparkline does not paint through the KPI value', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1280, height: 720});
		await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();

		const passStat = page.locator('.eval-stats .stat:has(.stat__label:has-text("Pass rate"))');
		const value = passStat.locator('.stat__value');
		const spark = passStat.locator('.stat__spark');
		await expect(value).toBeVisible();
		await expect(spark).toBeVisible();

		const collision = await passStat.evaluate(node => {
			const valueBox = node.querySelector('.stat__value')?.getBoundingClientRect();
			const sparkBox = node.querySelector('.stat__spark')?.getBoundingClientRect();
			if (!valueBox || !sparkBox) {
				return true;
		...
		expect(collision, 'Evals pass-rate sparkline should inspect trends without obscuring the KPI value').toBe(false);
	});
	...
	test('sparkline hover reports individual periods and clamps edge tooltips', async ({openConsole, page}) => {
		...
		await expect(tooltip).toContainText(/latest/i);
		await expect(tooltip).toContainText(/point 12\/12/i);
		await expect(tooltip).not.toContainText(/period 12\/12.*point 12\/12/i);
		await expect(tooltip).toContainText(/-0\.3pp vs prior/i);
		await expect(tooltip).not.toContainText(/0% vs prior/i);
	...
	test('suite trend sparkline tooltip is granular with day-based labels', async ({openConsole, page}) => {
		...
		const suiteTrendCard = page
			.locator('.card:has(.card__title:has-text("suite family · "))')
			.first();
		...
		await expect(tooltip).toContainText(/point 14\/14/i);
		await expect(tooltip).not.toContainText(/period 14\/14.*point 14\/14/i);
		await expect(tooltip).toContainText(/latest/i);
	});

	test('runs panel labels local eval artifacts without exposing fixture plumbing', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();

		// Card title now shows just the run count, not a confusing
		// `harness runs · loading` cram. The source badge is its own widget,
		// but the operator-facing text should describe local artifacts rather
		// than the static fixture implementation detail.
		const badge = page.locator('[data-testid="eval-runs-source-badge"]');
		await expect(badge).toBeVisible();
		const label = (await badge.textContent() || '').trim().toLowerCase();
		const state = (await badge.getAttribute('data-source-state') || '').trim().toLowerCase();
		// In the test harness the EvalsPage resolves to fixture (DEMO_MODE) or live.
		expect(['fixture', 'live']).toContain(state);
		expect(['local artifacts', 'live backend']).toContain(label);
		await expect(badge).not.toContainText(/fixture/i);
	...
	test('runs table renders per-row latency pill with budget-aware tone (no longer buried in metadata)', async ({openConsole}) => {
		...
		const snapshot = await page.evaluate(() => {
			...
			const pillRecords = pills.map(p => ({
				dur: (p as HTMLElement).dataset.durationMs,
				tone: (p as HTMLElement).dataset.tone,
		...
		for (const text of snapshot.metaTexts) {
			// Metadata should name the local agent wrapper + prompt profile without
			// leaking raw harness ids or prompt-version tags, and it must not end
			// with "· Ns" / "· Nms" because latency owns that pill.
			expect(text, `metadata row should not leak raw harness plumbing: "${text}"`).not.toMatch(/agent_[a-z]+_demo|prompt\/sewy\/v\d/i);
			expect(text, `metadata row should not duplicate the latency pill: "${text}"`).not.toMatch(/·\s*\d+(\.\d+)?(ms|s)\s*$/);
		}
	});

	test('Evals keeps raw harness prompt versions out of operator-facing copy', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		await expect(page.locator('.eval-run-row').first()).toBeVisible();

		const visibleCopy = await page.locator('.page--evals').innerText();
		expect(visibleCopy).toContain('active prompt profile');
		expect(visibleCopy).not.toMatch(/prompt\/sewy\/v\d/i);
		expect(visibleCopy).not.toMatch(/agent_[a-z]+_demo/i);

		await expect(page.locator('.eval-run-row .mono.dim').first()).toContainText(/Sales Coach|Sarah/);
		await expect(page.locator('.eval-meta-strip')).toContainText('active prompt profile');
	});
	...
	test('eval dashboard keeps the console route identity visible inside the command center', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();

		const masthead = page.getByTestId('eval-console-masthead');
		await expect(masthead).toBeVisible();
		await expect(masthead).toContainText(/loaded results/i);
		await expect(masthead).toContainText(/elevenlabs agents/i);

		const h1 = page.getByRole('heading', {level: 1, name: /^Evals$/});
		await expect(h1).toBeVisible();
		await expect(h1).toHaveAttribute('id', 'console-page-title');
		await expect(page.locator('main[aria-labelledby="console-page-title"] #console-page-title')).toHaveCount(1);

		const [h1Box, commandBox] = await Promise.all([
			h1.boundingBox(),
			page.getByTestId('eval-command-center').boundingBox(),
		]);
		expect(h1Box, 'eval route h1 should render visibly, not as a clipped sr-only node').not.toBeNull();
		expect(commandBox, 'eval command center should render').not.toBeNull();
		expect(h1Box!.width).toBeGreaterThan(20);
		expect(h1Box!.height).toBeGreaterThan(8);
		expect(h1Box!.y).toBeGreaterThanOrEqual(commandBox!.y);
		expect(h1Box!.y + h1Box!.height).toBeLessThan(commandBox!.y + commandBox!.height);
	});

	test('eval command center names evidence as a review packet without exposing fixture paths', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		...
		await expect(title).not.toContainText(/fixtures\/runs/i);
		await expect(evidence.getByTestId('eval-command-evidence-kind')).toContainText(/source evidence · json/i);
		await expect(source).toContainText(/source evidence/i);
		await expect(source).toContainText(/noisy-caller-transcription-stress/i);
	...
	test('eval command center stays pending while harness evidence is still loading', async ({page}) => {
		let releaseRuns!: () => void;
		const runsGate = new Promise<void>(resolve => {
			releaseRuns = resolve;
		});
		...
		await expect(page.getByTestId('eval-active-scenario-title')).toHaveText(/loading harness run evidence/i);
		await expect(page.getByTestId('eval-active-regression-review-copy')).toContainText(/waiting for a concrete harness run/i);
		await expect(page.getByTestId('eval-command-evidence-title')).toHaveText(/loading harness run evidence/i);
		await expect(page.getByTestId('eval-command-evidence-title')).not.toContainText(/no evidence packet linked/i);
		await expect(page.getByTestId('eval-command-evidence-kind')).toHaveText(/artifact loading/i);
		await expect(page.getByTestId('eval-command-evidence-source')).toHaveText(/waiting for \/api\/eval-runs/i);
		await expect(page.getByTestId('eval-command-open-artifact')).toBeDisabled();
		await expect(page.getByTestId('eval-command-open-admin')).toBeDisabled();
		await expect(page.locator('.eval-command-center__voice .el-orb')).toHaveClass(/el-orb--idle/);
		await expect(page.locator('.eval-command-center__voice .el-bars')).not.toHaveClass(/is-active/);
	...
	test('eval dashboard top surface stays compact at laptop width', async ({openConsole, page}) => {
		...
		const healthRowLayout = await page.getByTestId('eval-health-row').evaluate(row => {
			const rowBox = row.getBoundingClientRect();
			const latency = row.querySelector('[data-testid="eval-latency-strip"]')?.getBoundingClientRect();
			const tools = row.querySelector('[data-testid="eval-tool-latency-rollup"]')?.getBoundingClientRect();
			return {
				height: Math.round(rowBox.height),
				latencyTop: latency ? Math.round(latency.top) : null,
				toolsTop: tools ? Math.round(tools.top) : null,
			};
		});
		expect(
			healthRowLayout.height,
			`latency telemetry should read as one compact console strip: ${JSON.stringify(healthRowLayout)}`,
		).toBeLessThanOrEqual(74);
		expect(healthRowLayout.latencyTop).toBe(healthRowLayout.toolsTop);

		const toolRollupLayout = await page.getByTestId('eval-tool-latency-rollup').evaluate(rollup => {
			const rollupBox = rollup.getBoundingClientRect();
			const chips = [...rollup.querySelectorAll('[data-testid="eval-tool-latency-rollup-row"]')]
				.map(element => {
			...
			return {
				height: Math.round(rollupBox.height),
				overflowX: getComputedStyle(rollup as HTMLElement).overflowX,
				clientWidth: rollup.clientWidth,
				rootRight: Math.round(rollupBox.right),
				scrollWidth: rollup.scrollWidth,
				widestChip: Math.max(...chips.map(chip => chip.width)),
				chips,
			};
		});
		expect(
			toolRollupLayout.height,
			`tool latency strip should not become a second dashboard wall: ${JSON.stringify(toolRollupLayout)}`,
		).toBeLessThanOrEqual(72);
		expect(toolRollupLayout.overflowX).toBe('visible');
		expect(
			toolRollupLayout.scrollWidth,
			`tool latency strip should wrap instead of becoming a horizontal scroll trap: ${JSON.stringify(toolRollupLayout)}`,
		).toBeLessThanOrEqual(toolRollupLayout.clientWidth + 1);
		expect(
			toolRollupLayout.widestChip,
			`tool latency chips should stay bounded inside the dashboard column: ${JSON.stringify(toolRollupLayout)}`,
		).toBeLessThanOrEqual(toolRollupLayout.clientWidth + 1);
		expect(
			toolRollupLayout.chips.every(chip => chip.right <= toolRollupLayout.rootRight + 1),
			`tool latency chips should not paint past the viewport edge: ${JSON.stringify(toolRollupLayout)}`,
		).toBe(true);
		...
		const commandLayout = await page.locator('[data-testid="eval-command-center"]').evaluate(center => {
			const read = (selector: string) => {
			...
			const actionRows = [...center.querySelectorAll('.eval-command-center__actions .btn')]
				.map(node => Math.round((node as HTMLElement).getBoundingClientRect().top));
			return {
				actionsOneRow: new Set(actionRows).size <= 1,
				copy: read('.eval-command-center__copy'),
				evidence: read('[data-testid="eval-command-evidence-card"]'),
				voiceHead: read('.eval-command-center__voice-head'),
				ops: read('.eval-command-center__ops'),
				voice: read('.eval-command-center__voice'),
			};
		});
		expect(commandLayout.ops, `eval controls should render: ${JSON.stringify(commandLayout)}`).not.toBeNull();
		expect(commandLayout.copy, `eval copy should render: ${JSON.stringify(commandLayout)}`).not.toBeNull();
		expect(commandLayout.voice, `eval voice panel should render: ${JSON.stringify(commandLayout)}`).not.toBeNull();
		expect(commandLayout.evidence, `eval evidence card should render inside the local wrapper: ${JSON.stringify(commandLayout)}`).not.toBeNull();
		expect(commandLayout.voiceHead, `eval voice signal should render inside the local wrapper: ${JSON.stringify(commandLayout)}`).not.toBeNull();
		expect(
			commandLayout.ops!.left,
			`eval controls should align to the command center content edge: ${JSON.stringify(commandLayout)}`,
		).toBeLessThanOrEqual(commandLayout.copy!.left + 1);
		expect(
			commandLayout.ops!.right,
			`eval controls should span under the voice panel instead of squeezing under copy: ${JSON.stringify(commandLayout)}`,
		).toBeGreaterThanOrEqual(commandLayout.voice!.right - 1);
		expect(
			commandLayout.ops!.top,
			`eval controls should sit below the scenario copy and local wrapper: ${JSON.stringify(commandLayout)}`,
		).toBeGreaterThan(Math.max(commandLayout.copy!.bottom, commandLayout.voice!.bottom) - 1);
		expect(
			commandLayout.voice!.width,
			`eval local wrapper should have enough room for evidence/admin facts at laptop width: ${JSON.stringify(commandLayout)}`,
		).toBeGreaterThanOrEqual(380);
		expect(
			commandLayout.evidence!.top,
			`eval evidence card should sit below the live signal, not collide with it: ${JSON.stringify(commandLayout)}`,
		).toBeGreaterThan(commandLayout.voiceHead!.bottom - 1);
		expect(
			commandLayout.actionsOneRow,
			`eval command actions should fit in one row at laptop width: ${JSON.stringify(commandLayout)}`,
		).toBe(true);
	});

	test('eval command center keeps controls compact at tablet width', async ({openConsole, page}) => {
		await page.setViewportSize({width: 884, height: 768});
		await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();

		const layout = await page.locator('[data-testid="eval-command-center"]').evaluate(center => {
			...
			const actions = [...center.querySelectorAll('.eval-command-center__actions .btn')] as HTMLElement[];
			const actionRows = actions.map(node => Math.round(node.getBoundingClientRect().top));
			const actionWidths = actions.map(node => Math.round(node.getBoundingClientRect().width));
			const evidencePath = center.querySelector<HTMLElement>('.eval-command-center__evidence code');
			const evidenceActions = center.querySelector<HTMLElement>('.eval-command-center__evidence-actions');
			const scrollHost = document.querySelector('.scroll');
			const stats = [...document.querySelectorAll('.eval-stats .stat')] as HTMLElement[];
			const statRows = stats.map(node => Math.round(node.getBoundingClientRect().top));
			return {
				actionRows: new Set(actionRows).size,
				actionWidths,
				center: rectOf(center),
				evidenceActions: rectOf(evidenceActions),
				evidencePath: rectOf(evidencePath),
				scrollClientWidth: scrollHost?.clientWidth || 0,
				scrollWidth: scrollHost?.scrollWidth || 0,
				statRowCount: new Set(statRows).size,
				pathClientWidth: evidencePath?.clientWidth || 0,
				pathScrollHeight: evidencePath?.scrollHeight || 0,
				pathClientHeight: evidencePath?.clientHeight || 0,
				pathWhiteSpace: evidencePath ? getComputedStyle(evidencePath).whiteSpace : '',
			};
		});

		expect(layout.center, `eval command center should render: ${JSON.stringify(layout)}`).not.toBeNull();
		expect(layout.scrollWidth, `tablet eval page should not expose horizontal scrolling: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(layout.scrollClientWidth + 1);
		expect(layout.statRowCount, `tablet eval KPI tiles should wrap into two scannable rows: ${JSON.stringify(layout)}`).toBe(2);
		expect(layout.center!.height, `tablet eval command center should stay bounded while keeping evidence readable: ${JSON.stringify(layout)}`).toBeLessThan(430);
		expect(layout.actionRows, `tablet eval actions should wrap as one compact row, not one button per row: ${JSON.stringify(layout)}`).toBe(1);
		expect(Math.max(...layout.actionWidths), `tablet eval action buttons should not expand to the full card width: ${JSON.stringify(layout)}`).toBeLessThan(170);
		expect(layout.evidencePath, `eval evidence path should render inside the local wrapper: ${JSON.stringify(layout)}`).not.toBeNull();
		expect(layout.evidenceActions, `eval evidence actions should render inside the local wrapper: ${JSON.stringify(layout)}`).not.toBeNull();
		expect(layout.evidencePath!.width, `tablet evidence path should get real reading width instead of a tiny side rail: ${JSON.stringify(layout)}`).toBeGreaterThan(300);
		expect(layout.pathWhiteSpace, `evidence path should wrap only when needed: ${JSON.stringify(layout)}`).toBe('normal');
		expect(layout.pathScrollHeight, `evidence path should not be clipped vertically: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(layout.pathClientHeight + 1);
		expect(layout.evidenceActions!.top, `evidence actions should sit under the path, not squeeze it horizontally: ${JSON.stringify(layout)}`).toBeGreaterThan(layout.evidencePath!.bottom - 1);
	});
	...
	test('eval policy action opens Settings on the Eval policy tab', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		const policySettings = page.getByTestId('eval-policy-settings');
		await expect(policySettings).toHaveText(/policy settings/i);
		await expect(policySettings).toHaveAttribute('aria-label', /open eval policy settings/i);
		await expect(page.getByRole('button', {name: /^policy$/i})).toHaveCount(0);
		await policySettings.click();
	...
	test('eval New suite stays native, creates a draft row, and opens the harness run plan', async ({openConsole}) => {
		...
		const newSuiteButton = page.getByRole('button', {name: /^new suite$/i});
		await expect(newSuiteButton).toHaveAttribute('aria-controls', 'eval-suite-builder');
		await expect(newSuiteButton).toHaveAttribute('aria-expanded', 'false');
		await newSuiteButton.click();
		const builder = page.locator('[data-testid="eval-suite-builder"]');
		await expect(builder).toBeVisible();
		await expect(newSuiteButton).toHaveAttribute('aria-expanded', 'true');
		await expect(builder.getByLabel('Suite name')).toBeFocused();
		await expect(page.locator('.tb__crumb--active')).toContainText('Evals');
		await expect(page).toHaveURL(/route=evals/);

		await builder.getByRole('button', {name: /add suite draft/i}).click();
		await expect(builder.getByRole('alert')).toContainText(/suite name and scenario focus are required/i);
		await expect(builder.getByLabel('Suite name')).toBeFocused();
		await expect(builder.getByLabel('Suite name')).toHaveAttribute('aria-invalid', 'true');

		await builder.getByLabel('Suite name').fill('Refund objection smoke');
		await builder.getByRole('button', {name: /add suite draft/i}).click();
		await expect(builder.getByLabel('Scenario focus')).toBeFocused();
		await expect(builder.getByLabel('Scenario focus')).toHaveAttribute('aria-invalid', 'true');
	...
	test('eval New suite builder keeps submit controls in the first tablet viewport', async ({openConsole, page}) => {
		await page.setViewportSize({width: 884, height: 768});
		await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		...
		const layout = await builder.evaluate(element => {
			const rowsFor = (selector: string) => {
				const tops = [...element.querySelectorAll(selector)].map(node =>
					Math.round((node as HTMLElement).getBoundingClientRect().top));
			...
			const scrollHost = document.querySelector('.scroll');
			return {
				actions: rect('.eval-suite-builder__actions'),
				builder: rect(':scope'),
				fieldRows: rowsFor('.eval-suite-builder__grid > .field'),
				gridColumns: getComputedStyle(element.querySelector('.eval-suite-builder__grid')!).gridTemplateColumns,
				scrollClientWidth: scrollHost?.clientWidth || 0,
				scrollWidth: scrollHost?.scrollWidth || 0,
		...
		expect(layout.fieldRows, `builder should not collapse into five stacked rows: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(3);
		expect(layout.gridColumns.split(' ').length, `builder should use tablet columns: ${JSON.stringify(layout)}`).toBeGreaterThanOrEqual(2);
		expect(layout.actions, `builder actions should render: ${JSON.stringify(layout)}`).not.toBeNull();
		expect(layout.actions!.bottom, `submit controls should stay in the first viewport: ${JSON.stringify(layout)}`).toBeLessThan(layout.viewportHeight);
		expect(layout.scrollWidth, `builder should not create horizontal scrolling: ${JSON.stringify(layout)}`).toBeLessThanOrEqual(layout.scrollClientWidth + 1);
	});

	test('eval run-plan drawer mirrors real manifest commands without leaking the satellite manifest name', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();

		// Status line reflects the command count from the manifest, but the
		// first-viewport console summary should read like a local artifact.
		const status = page.locator('[data-testid="eval-harness-manifest-status"]');
		await expect(status).toContainText(/console eval manifest/i, {timeout: 10_000});
		await expect(status).toContainText(/8 commands/);
		await expect(status).not.toContainText(/voice_ai_agent_evals|eval-harness\.manifest\.json/);
		...
		const commandArtifact = detail.locator('[data-testid="eval-harness-artifact"]').filter({hasText: /Playwright console report/});
		await expect(commandArtifact).toContainText('playwright-console-report/index.html');
		await expect(commandArtifact.locator('a')).toHaveCount(0);
		await page.evaluate(() => {
			Object.defineProperty(navigator, 'clipboard', {
				configurable: true,
				value: {
					async writeText(text: string) {
						(globalThis as any).__copiedHarnessArtifactPath = text;
					},
				},
			});
		});
		await commandArtifact.getByRole('button', {name: /copy playwright console report evidence reference/i}).click();
		await expect.poll(async () => page.evaluate(() => (globalThis as any).__copiedHarnessArtifactPath)).toBe('playwright-console-report/index.html');
		await expect(page.locator('.toast', {hasText: /evidence reference copied/i})).toBeVisible();
		await expect(page.locator('.toast', {hasText: /artifact path copied/i})).toHaveCount(0);
		const openArtifact = detail.getByRole('button', {name: /open review evidence/i});
...
test.describe('agents page', () => {
	...
	test('playground widget frame includes a local session packet instead of a blank embed box', async ({openConsole}) => {
		...
		await expect(session).toContainText(/local wrapper/i);
		await expect(session).toContainText(/route\s+agents/i);
		await expect(session).toContainText(/context\s+\d+ line(s)?/i);
		await expect(session.getByTestId('agent-session-tools')).toHaveText('3 local');
		await expect(frame.locator('elevenlabs-convai')).toHaveCount(1, {timeout: 10_000});
	...
	test('admin-only agent session packet does not invent local tools', async ({page}) => {
		await page.addInitScript(() => {
			(globalThis as any).DEMO_MODE = true;
		});
		await page.goto('/console/?admin=1&route=agents', {waitUntil: 'domcontentloaded'});
		await page.waitForFunction(() => Boolean(document.querySelector('.app')), null, {timeout: 30_000});

		await page.locator('.agents-grid .vstack [role="button"]').filter({hasText: /client data test/i}).first().click();
		const frame = page.locator('[data-testid="agent-playground-convai"]');
		const session = frame.locator('.agent-session-strip');
		await expect(session).toContainText(/client data test/i);
		await expect(session).toContainText(/local wrapper/i);
		await expect(session.getByTestId('agent-session-tools')).toHaveText('none declared');
		await expect(session).not.toContainText(/tools\s+3 local/i);

		await page.locator('.agent-admin-tab:has-text("Tools")').click();
		await expect(page.locator('[data-testid="agent-tools-empty"]')).toBeVisible();
	});
