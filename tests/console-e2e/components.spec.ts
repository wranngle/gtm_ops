...
test.describe('shell', () => {
	test('sidebar nav toggles every route', async ({openConsole}) => {
		...
		const routes = [
			{id: 'home', crumb: /callbacks/i},
			{id: 'generate', crumb: /generate/i},
			{id: 'pipeline', crumb: /pipeline/i},
			{id: 'calls', crumb: /calls/i},
			{id: 'proposals', crumb: /proposals/i},
			{id: 'evals', crumb: /evals/i},
			{id: 'agents', crumb: /agents/i},
			{id: 'settings', crumb: /settings/i},
		];
	...
	test('sidebar agent row highlights as active while route=agents AND mirrors the in-page picker switch', async ({openConsole}) => {
		...
		await page.locator('[data-testid="agents-picker-row"][data-agent-key="intake"]').click();
	...
	test('sidebar brand shows Wranngle / gtm_ops without exposing a version', async ({openConsole}) => {
		...
		await expect(page.locator('.sb__brand .sb__wordmark-text')).toHaveText('Wranngle');
		...
		await expect(page.getByTestId('topbar-route-label')).toContainText(/wranngle\s*\/\s*gtm_ops console/i);
		await expect(page.getByTestId('topbar-route-label')).not.toContainText(/v\d/i);
	});
	...
	test('icon-label buttons keep readable word and icon spacing across dense console surfaces', async ({openConsole}) => {
		...
		await assertReadableButton(page.getByRole('button', {name: /inspect pdf requirements/i}), 'Generate artifact PDF button');
		...
		await page.locator('.sb__item:has-text("Agents")').first().click();
		await assertReadableButton(page.getByRole('button', {name: /local elevenlabs workspace settings/i}), 'Agents local workspace settings button');
		await page.getByTestId('agents-workspace-settings').click();
		const elevenLabsConfig = page.getByRole('region', {name: /^elevenlabs configuration$/i});
		await expect(elevenLabsConfig.locator('a[href*="elevenlabs.io"], a[href*="elevenlabs.com"]')).toHaveCount(1);
		await assertReadableButton(elevenLabsConfig.getByTestId('integration-open-elevenlabs-local-admin'), 'Settings ElevenLabs local admin button');
		await assertReadableButton(elevenLabsConfig.getByRole('link', {name: /open elevenlabs agents dashboard/i}), 'Settings ElevenLabs explicit escape hatch');
	});
	...
	test('sparkline inspection is granular without flooding the tab order', async ({openConsole}) => {
		...
		await expect(firstSparkline.locator('.spark-tooltip')).toContainText(/waiting on you trend/i);
		await expect(firstSparkline.locator('.spark-tooltip')).toContainText(/latest/i);
		await expect(firstSparkline.locator('.spark-crosshair')).toBeVisible();
		const latestCrosshairLeft = await firstSparkline.locator('.spark-crosshair').evaluate(element => getComputedStyle(element).left);
		await page.keyboard.press('ArrowLeft');
		await expect(firstSparkline.locator('.spark-tooltip')).toContainText(/ago/i);
		await expect(firstSparkline.locator('.spark-crosshair')).not.toHaveCSS('left', latestCrosshairLeft);
	...
	test('compact sidebar keeps only the lasso mark and hides clipped agent surface labels', async ({openConsole, page}) => {
		...
		await expect(page.locator('.sb__brand .sb__wordmark-text')).toBeHidden();
	...
	test('route query deep-links directly into console pages', async ({page}) => {
		...
		await expect(page.locator('h1#console-page-title.ph__title')).toContainText('Evals');
	});
});
...
test.describe('topbar', () => {
	test('command palette opens, filters, and dispatches', async ({openConsole}) => {
		...
		await expect(calls).toHaveJSProperty('tagName', 'BUTTON');
		await calls.click();
		await expect(page.locator('.tb__crumb--active')).toContainText('Calls');
	});

	test('command palette exposes visible commands as native buttons', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.tb__search').click();
		const cp = page.locator('.cp');
		await expect(cp).toBeVisible();
		const input = cp.locator('.cp__input');
		...
		const callButton = page.getByRole('button', {name: /go to calls/i});
	...
	test('Generate review path exposes explicit artifact and proposal navigation', async ({openConsole}) => {
		...
		const pendingArtifactAction = page.locator('[data-testid="generate-review-path-action-inspect-artifact-requirements"]');
		await expect(pendingArtifactAction).toBeVisible();
		await expect(pendingArtifactAction).toContainText(/inspect requirements/i);
		await pendingArtifactAction.click();
		const drawer = page.locator('[role="region"][aria-label="Proposal artifact review drawer"]');
		await expect(drawer).toBeVisible();
		await expect(drawer).toContainText(/run sequence required before render/i);
...
test.describe('mission control', () => {
	...
	test('Home mission stats stay callback-scoped instead of leaking eval telemetry', async ({openConsole}) => {
		const page = await openConsole();
		await expect.poll(async () => page.evaluate(() => {
			const stats = [...document.querySelectorAll('[data-testid="mission-stats"] .stat')];
			const labels = stats.map(s => (s.querySelector('.stat__label')?.textContent || '').trim());
			return {
				labels,
				hasEvalTelemetry: labels.some(label => /eval|pass rate|harness|regression/i.test(label)),
				hasRecoveryMetric: labels.some(label => /^returned within 1h$/i.test(label)),
			};
		}).then(({labels, hasEvalTelemetry, hasRecoveryMetric}) => (
			labels.length === 5 && !hasEvalTelemetry && hasRecoveryMetric
		))).toBe(true);
	});
	...
	test('sparkline metrics expose a hover popout', async ({openConsole}) => {
		const page = await openConsole();
		const spark = page.locator('.stat:has(.stat__label:has-text("Returned within 1h")) .spark-wrap');
		...
		await expect(spark.locator('.spark-tooltip')).toContainText('Returned within 1h rate trend');
		await expect(spark.locator('.spark-tooltip')).toContainText('%');
		await expect(spark.locator('.spark-tooltip')).toContainText(/vs prior|baseline/);
	...
	test('sparkline charts do not collide with callback KPI values', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1366, height: 850});
		await openConsole();

		const stat = page.locator('.stat:has(.stat__label:has-text("Waiting on you"))').first();
		...
		expect(collision, 'callback KPI sparklines should reserve their own space instead of painting through the value').toBe(false);
	});

	test('sparkline hover preserves absolute-count deltas', async ({openConsole}) => {
		const page = await openConsole();

		const spark = page.locator('.stat:has(.stat__label:has-text("Missed · today")) .spark-wrap');
		await expect(spark).toBeVisible();

		await spark.locator('.spark-point').nth(7).hover({force: true});
		await expect(spark.locator('.spark-tooltip')).toContainText('Missed · today trend');
		await expect(spark.locator('.spark-tooltip')).toContainText('2');
	...
	test('empty sparkline series render as honest no-data states', async ({openConsole}) => {
		const page = await openConsole();

		await page.evaluate(() => {
			const host = document.createElement('div');
			host.id = 'empty-spark-host';
			document.body.append(host);
			const root = (globalThis as any).ReactDOM.createRoot(host);
			root.render((globalThis as any).React.createElement((globalThis as any).Sparkline, {
				data: [],
				label: 'Empty callback trend',
			}));
		});

		const spark = page.locator('#empty-spark-host .spark-wrap');
		await expect(spark).toBeVisible();
		await expect(spark).toHaveClass(/spark-wrap--empty/);
		await expect(spark).toHaveAttribute('role', 'img');
		await expect(spark).toHaveAttribute('aria-label', 'Empty callback trend: no trend data available');
		expect(await spark.getAttribute('tabindex')).toBeNull();
		await expect(spark.locator('.spark-point')).toHaveCount(0);
		await expect(spark.locator('.spark-tooltip')).toHaveCount(0);

		const html = await spark.evaluate(node => node.outerHTML);
		expect(html).not.toMatch(/undefined|NaN|Infinity/);
	});

	test('callbacks owed KPI carries trend evidence instead of dead space', async ({openConsole}) => {
		const page = await openConsole();

		const qualifiedStat = page.locator('.stat:has(.stat__label:has-text("Callbacks owed · today"))');
		...
		await expect(spark.locator('.spark-tooltip')).toContainText('Callbacks owed · today trend');
	...
	test('sparkline point labels are granular without floating-point noise', async ({openConsole}) => {
		...
		expect(labels.some(label => label.includes('Returned within 1h rate trend'))).toBe(true);
		expect(labels.some(label => label.includes('Callbacks owed · today trend'))).toBe(true);
		expect(labels.some(label => label.includes('Missed · today trend'))).toBe(true);
		expect(labels.join('\n')).toContain('ago');
		expect(labels.join('\n')).not.toMatch(/\d+\.\d{3,}/);
	...
	test('stat deltas normalize signs without duplicate pluses', async ({openConsole}) => {
		const page = await openConsole();
		const waitingDelta = page.locator('.stat:has(.stat__label:has-text("Waiting on you")) .stat__delta');
		await expect(waitingDelta).toContainText('+8 customers vs last week');
		await expect(waitingDelta).not.toContainText('++');

		const returnedDelta = page.locator('.stat:has(.stat__label:has-text("Returned within 1h")) .stat__delta');
		await expect(returnedDelta).toContainText('+0.3 vs last week');
		await expect(returnedDelta).not.toContainText('+0.3%');
	});
});
...
test.describe('pipeline', () => {
	...
	test('Agents page has one explicit ElevenLabs escape hatch; recovery actions stay local', async ({openConsole}) => {
		...
		await page.locator('[data-testid="agents-workspace-settings"]').click();
		const elevenLabsConfig = page.getByRole('region', {name: /^elevenlabs configuration$/i});
		...
		const localAdmin = elevenLabsConfig.locator('[data-testid="integration-open-elevenlabs-local-admin"]');
	...
	test('lead artifacts open in-console instead of a raw fixture tab', async ({openConsole, context}) => {
		...
		await expect(drawer).toBeVisible();
		await expect(drawer).toContainText(/review locator/i);
		await expect(drawer.locator('[data-testid="lead-artifact-locator"]')).toContainText('local-review://pipeline/acme-hvac-r3/schema.json');
		await expect(drawer).not.toContainText(/repo path|copy repo path|fixtures\//i);
		await expect(drawer.locator('.lead-artifact-row').first()).toContainText('local-review://pipeline/acme-hvac-r3/schema.json');
		await expect(drawer.locator('pre')).toContainText(/tr_demo_acme_001/);

		await drawer.getByRole('button', {name: /copy review locator/i}).click();
		await expect(page.locator('.toast', {hasText: /review locator copied/i})).toContainText('local-review://pipeline/acme-hvac-r3/schema.json');
...
test.describe('proposals', () => {
	...
	test('"Review packet" proposal opens a selectable local artifact viewer', async ({openConsole, page}) => {
		await page.setViewportSize({width: 1280, height: 720});
		await openConsole();
		await page.locator('.sb__item:has-text("Proposals")').first().click();
		...
		await expect(panel).toBeVisible();
		await expect(page.getByRole('region', {name: 'Proposal workflow panel'})).toBeFocused();
		const geometry = await page.getByRole('region', {name: 'Proposal workflow panel'}).evaluate(region => {
			const scroller = document.querySelector('main.scroll');
			const firstArtifact = region.querySelector('[data-testid="proposal-review-artifact"]');
			const scrollBox = scroller?.getBoundingClientRect();
			const regionBox = region.getBoundingClientRect();
			const artifactBox = firstArtifact?.getBoundingClientRect();
			return {
				firstArtifactBottom: artifactBox?.bottom ?? 0,
				firstArtifactTop: artifactBox?.top ?? 0,
				regionTop: regionBox.top,
				scrollerTop: scrollBox?.top ?? 0,
				viewportHeight: window.innerHeight,
			};
		});
		expect(geometry.regionTop, `workflow region should scroll below the topbar: ${JSON.stringify(geometry)}`).toBeGreaterThanOrEqual(geometry.scrollerTop + 10);
		expect(geometry.firstArtifactTop, `artifact tabs should not be clipped under the topbar: ${JSON.stringify(geometry)}`).toBeGreaterThanOrEqual(geometry.scrollerTop + 10);
		expect(geometry.firstArtifactBottom, `first artifact tab should be visible after opening review: ${JSON.stringify(geometry)}`).toBeLessThanOrEqual(geometry.viewportHeight);
		...
		await expect(panel.locator('[data-testid="proposal-pdf-bound-preview"]')).toBeVisible();
		await expect(panel.locator('[data-testid="proposal-review-pdf-title"]')).toHaveText('Banyan Health');
		await expect(panel.locator('[data-testid="proposal-review-artifact-preview"] iframe')).toHaveCount(0);
		await expect(panel).not.toContainText(/Acme HVAC Services/);
		...
		await expect(panel.locator('[data-testid="proposal-review-source-json"]')).toContainText(/"proposal_id": "pr-2041"/i);
		await expect(panel.locator('[data-testid="proposal-review-source-json"]')).toContainText(/"buyer": "banyan health"/i);
		await expect(panel.locator('[data-testid="proposal-review-artifact-preview"] iframe')).toHaveCount(0);
	});
});
...
test.describe('evals', () => {
	...
	test('Evals artifacts action replaces an open run-plan popout', async ({openConsole, page}) => {
		...
		await page.locator('.ph__actions').getByRole('button', {name: /^run plan$/i}).click();
	...
	test('stats reflect loaded harness run health instead of hard-coded healthy demo values', async ({openConsole}) => {
		...
		await expect(passStat.locator('.stat__value')).toHaveClass(/stat__value--warn/);
		await expect(passStat.locator('.spark-point')).toHaveCount(8);
		await expect(passStat.locator('.spark-wrap')).toHaveAttribute('aria-label', /5\/8 loaded harness results/i);
		await expect(passStat.locator('.spark path[stroke]').first()).toHaveAttribute('stroke', 'var(--warning)');
		const passStatGeometry = await passStat.evaluate(element => {
			const labelNode = element.querySelector('.stat__label');
			const valueNode = element.querySelector('.stat__value');
			const sparkNode = element.querySelector<HTMLElement>('.stat__spark');
			const label = labelNode?.getBoundingClientRect();
			const value = valueNode?.getBoundingClientRect();
			const spark = sparkNode?.getBoundingClientRect();
			if (!label || !value || !spark || !labelNode || !valueNode || !sparkNode) {
				return null;
			}
			const sparkWrap = sparkNode.querySelector<HTMLElement>('.spark-wrap');
			if (!sparkWrap) {
				return null;
			}
			...
			return {
				...
				valueFits: valueNode.scrollWidth <= valueNode.clientWidth + 1,
				sparkFits: sparkNode.scrollWidth <= sparkNode.clientWidth + 1,
				wrapFits: sparkWrap.scrollWidth <= sparkWrap.clientWidth + 1,
				pointsInside: [...sparkNode.querySelectorAll('.spark-point')].every(point => {
					const pointBox = point.getBoundingClientRect();
					return pointBox.left >= spark.left - 1 && pointBox.right <= spark.right + 1;
				}),
				labelOverlap: overlaps(label, spark),
				valueOverlap: overlaps(value, spark),
			};
		});
		expect(passStatGeometry).not.toBeNull();
		expect(passStatGeometry!.labelText).toBe('Pass rate');
		expect(passStatGeometry!.labelFits, 'Pass rate label should render fully, not as PASS R...').toBe(true);
		expect(passStatGeometry!.valueText).toBe('63%');
		expect(passStatGeometry!.valueFits, 'Pass rate value should fit next to the sparkline').toBe(true);
		expect(passStatGeometry!.sparkFits, 'Pass rate sparkline hit targets should not create internal horizontal overflow').toBe(true);
		expect(passStatGeometry!.wrapFits, 'Pass rate sparkline wrap should stay inside the stat slot').toBe(true);
		expect(passStatGeometry!.pointsInside, 'Pass rate edge points should remain inside the stat sparkline slot').toBe(true);
		expect(passStatGeometry!.labelOverlap, 'Pass rate label should not collide with the sparkline').toBe(false);
		expect(passStatGeometry!.valueOverlap, 'Pass rate value should not collide with the sparkline').toBe(false);
	...
	test('sparkline hover reports individual periods and clamps edge tooltips', async ({openConsole, page}) => {
		...
		await expect(tooltip).toContainText(/loaded run/i);
		await expect(tooltip).toContainText(/\d+\.\d%/);
		await expect(tooltip).toContainText(/pp vs prior/i);
	...
	test('suite trend sparkline tooltip is granular with day-based labels', async ({openConsole, page}) => {
		...
		await expect(tooltip).toContainText(/latest/i);
		await expect(tooltip).toContainText(/87\.2%/);
		await expect(tooltip).toContainText(/pp vs prior/i);
	});
	...
	test('runs table renders per-row latency pill with budget-aware tone (no longer buried in metadata)', async ({openConsole}) => {
		...
		const snapshot = await page.evaluate(() => {
			const pills = [...document.querySelectorAll<HTMLElement>('[data-testid="eval-run-row-latency"]')];
	...
	test('eval dashboard surfaces the harness latency dimension (avg + slowest + budget)', async ({openConsole}) => {
		...
		const slowestScenario = strip.locator('[data-testid="eval-slowest-scenario"]');
		await expect(slowestScenario).not.toBeEmpty();
		await expect(slowestScenario).toHaveText(/^[A-Z][A-Za-z ]+$/u);
		await expect(slowestScenario).not.toContainText(/-/u);
		await expect(slowestScenario).toHaveAttribute('data-scenario-id', /[a-z0-9\-]+/u);
	...
	test('coach launcher stays in shell chrome on /evals after opening a run detail', async ({openConsole, page}) => {
		// Evals owns its own local evidence/admin controls. Keep the global
		// Sales Coach pill in the shell chrome on this route instead of
		// letting it float over run-plan or evidence review actions.
		await page.setViewportSize({width: 1280, height: 900});
		await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		...
		const launcher = page.locator('.coach-launcher');
		const topbar = page.locator('.tb');
		const search = page.locator('.tb__search');
		const runPlanButton = page.locator('[data-testid="eval-run-plan-open"]').first();
		await expect(launcher).toBeVisible();
		await expect(topbar).toBeVisible();
		await expect(search).toBeVisible();
		await expect(runPlanButton).toBeVisible();
		const [box, topbarBox, searchBox, runPlanBox] = await Promise.all([
			launcher.boundingBox(),
			topbar.boundingBox(),
			search.boundingBox(),
			runPlanButton.boundingBox(),
		]);
		const overlaps = (a: NonNullable<typeof box>, b: NonNullable<typeof box>) =>
			a.x < b.x + b.width
			&& a.x + a.width > b.x
			&& a.y < b.y + b.height
			&& a.y + a.height > b.y;
		expect(box, 'coach launcher should render').not.toBeNull();
		expect(topbarBox, 'topbar should render').not.toBeNull();
		expect(searchBox, 'command search should render').not.toBeNull();
		expect(runPlanBox, 'eval run-plan button should render').not.toBeNull();
		expect(box!.y, 'eval launcher should stay inside shell chrome').toBeGreaterThanOrEqual(topbarBox!.y);
		expect(box!.y + box!.height, 'eval launcher should stay inside shell chrome').toBeLessThanOrEqual(topbarBox!.y + topbarBox!.height + 1);
		expect(box!.x + box!.width, 'launcher should leave command search clickable').toBeLessThanOrEqual(searchBox!.x - 8);
		expect(overlaps(box!, runPlanBox!), 'launcher should not cover the visible eval run-plan action').toBe(false);
		expect(box!.x, 'launcher should remain inside the main shell, not the sidebar').toBeGreaterThan(topbarBox!.x);
	});
	...
	test('eval New suite stays native, creates a draft row, and opens the harness run plan', async ({openConsole}) => {
		const page = await openConsole();
		await page.locator('.sb__item:has-text("Evals")').first().click();
		await expect(page.locator('[data-testid="eval-harness-manifest-status"]')).toContainText(/8 local commands/, {timeout: 10_000});
		...
		await expect(builder.locator('.field__label', {hasText: /^Suite name$/})).toHaveCount(1);
		await expect(builder.getByLabel('Suite name')).toHaveAttribute('placeholder', 'Name this suite');
		await expect(builder.getByLabel('Scenario focus')).toHaveAttribute('placeholder', 'Describe caller behavior and success criteria');
		...
		await expect(page.locator('[data-testid="eval-command-center"]')).toContainText(/draft suite queued/i);
		await expect(page.locator('[data-testid="eval-command-center"]')).toContainText(/refund objection smoke/i);
		await expect(page.locator('[data-testid="eval-command-center"]')).not.toContainText(/noisy caller transcription stress/i);
		await expect(page.locator('[data-testid="eval-command-center-review-evidence"]')).toHaveCount(0);
		await expect(page.locator('[data-testid="eval-run-plan-open-artifact"]')).toBeDisabled();
	...
	test('eval run-plan drawer mirrors the real eval-harness.manifest.json (no fake adapter command)', async ({openConsole}) => {
		...
		await expect(status).toHaveAttribute('data-schema-version', 'voice_ai_agent_evals.gtm_ops.v1', {timeout: 10_000});
		await expect(status).toContainText(/8 local commands/);
...
test.describe('agents page', () => {
	...
	test('agents header keeps local settings primary and demotes the ElevenLabs escape hatch', async ({openConsole}) => {
		...
		await expect(settings).toHaveAccessibleName(/local elevenlabs workspace settings/i);
		...
		const elevenLabsConfig = page.getByRole('region', {name: /^elevenlabs configuration$/i});
		await expect(elevenLabsConfig).toBeVisible();
		await expect(elevenLabsConfig).toContainText(/one explicit dashboard escape hatch/i);
		const escapeHatch = elevenLabsConfig.locator('[data-testid="integration-elevenlabs-escape"]');
		await expect(escapeHatch).toHaveClass(/btn--external/);
		await expect(escapeHatch).not.toHaveClass(/btn--primary/);
		await expect(escapeHatch).toHaveAccessibleName(/open elevenlabs agents dashboard in a new tab/i);
...
test.describe('settings', () => {
	...
	test('every integration configure/connect button opens details, including Clay Krisp and ElevenLabs', async ({openConsole}) => {
		...
		for (let i = 0; i < count; i += 1) {
			...
			const openButton = row.locator('[data-testid="integration-open"]');
			await expect(openButton).toHaveAccessibleName(new RegExp(`^(Configure|Connect) ${name} integration$`, 'i'));
