...
test('public shell keeps ElevenLabs agents visible as local console targets', async ({openConsole}) => {
	...
	const labelMetrics = await agentRows.evaluateAll(rows => rows.map(row => {
		...
		return {
			...
			scopeStartsBelowName: Boolean(nameBox && surfaceBox && surfaceBox.top >= nameBox.bottom - 1),
	...
	for (const metrics of labelMetrics) {
		...
		expect(metrics.scopeStartsBelowName, 'agent scope should read as a separate line, not glued to the name').toBe(true);
