 ...
 *
 *   UI fetch path  ->  server.js registration line  ->  HTTP status
 *
 * Usage:
 *   bun scripts/route_coverage_probe.js                  # assumes server already running on :3000
 *   bun scripts/route_coverage_probe.js --start          # starts `bun --env-file=.env server.js` itself
 *   bun scripts/route_coverage_probe.js --out docs/route-coverage.md
 *
 * Notes:
...
async function startServer() {
  const proc = spawn('bun', ['--env-file=.env', 'server.js'], {
    cwd: REPO_ROOT,
...
function renderTable(rows) {
  const header = [
    '| UI fetch path | Method | server.js route | Line | HTTP |',
    '| --- | --- | --- | --- | --- |',
  ];
  const body = rows.map(
    (r) =>
...
async function main() {
  ...
  const md = [
    ...
    '',
    `Generated by \`scripts/route_coverage_probe.js\` against \`${HOST}\` in DEMO_MODE.`,
